...
voidpf ZAlloc(voidpf opaque, uInt items, uInt size) { return (MemAlloc(items * size)); }

void ZFree(voidpf opaque, voidpf address) { MemFree(address); }
...
UINT32 CompressedBufferSize(
    UINT32 uiDataSize) {  // Function that calculates the worst-case buffer size needed to
...
UINT32 Compress(PTR pCompPtr, BYTE *pBuffer, UINT32 uiBufferLen) {
  ...
  if (pZStream->avail_in == 0) {  // There is nothing left to compress!
    return (0);
  }
