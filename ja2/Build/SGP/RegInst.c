...
HKEY GetAppRegistryKey() {
  ...
  if (RegOpenKeyEx(HKEY_CURRENT_USER, szSoftware, 0, KEY_WRITE | KEY_READ, &hSoftKey) ==
      ERROR_SUCCESS) {
    DWORD dw;
    if (RegCreateKeyEx(hSoftKey, gszRegistryKey, 0, REG_NONE, REG_OPTION_NON_VOLATILE,
                       KEY_WRITE | KEY_READ, NULL, &hCompanyKey, &dw) == ERROR_SUCCESS) {
      RegCreateKeyEx(hCompanyKey, gszProfileName, 0, REG_NONE, REG_OPTION_NON_VOLATILE,
                     KEY_WRITE | KEY_READ, NULL, &hAppKey, &dw);
    }
  }
...
HKEY GetSectionKey(STR lpszSection) {
  ...
  RegCreateKeyEx(hAppKey, lpszSection, 0, REG_NONE, REG_OPTION_NON_VOLATILE, KEY_WRITE | KEY_READ,
                 NULL, &hSectionKey, &dw);
  RegCloseKey(hAppKey);
  return hSectionKey;
}
...
UINT GetProfileInteger(STR lpszSection, STR lpszEntry, int nDefault) {
  ...
  if (gszRegistryKey[0] != '\0')  // use registry
  {
    ...
    lResult =
        RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, (LPBYTE)&dwValue, &dwCount);
    RegCloseKey(hSecKey);
...
BOOLEAN GetProfileChar(STR lpszSection, STR lpszEntry, STR lpszDefault, STR lpszValue) {
  ...
  if (gszRegistryKey[0] != '\0') {
    ...
    if (lResult == ERROR_SUCCESS) {
      ...
      lResult =
          RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, (LPBYTE)strValue, &dwCount);
    }
    RegCloseKey(hSecKey);
    if (lResult == ERROR_SUCCESS) {
      assert(dwType == REG_SZ);
...
BOOL WriteProfileChar(STR lpszSection, STR lpszEntry, STR lpszValue) {
  ...
  if (gszRegistryKey[0] != '\0') {
    ...
    if (lpszEntry == NULL)  // delete whole section
    {
      ...
      HKEY hSecKey = GetSectionKey(lpszSection);
      if (hSecKey == NULL) return FALSE;
      lResult = RegSetValueEx(hSecKey, lpszEntry, 0, REG_SZ, (LPBYTE)lpszValue,
