...
BOOLEAN EnterMutex(UINT32 uiMutexIndex, INT32 nLine, char *szFilename) {
  switch (WaitForSingleObject(MutexTable[uiMutexIndex], INFINITE)) {
    case WAIT_OBJECT_0:
      return TRUE;
    case WAIT_TIMEOUT:
      DbgMessage(TOPIC_MUTEX, DBG_LEVEL_0,
                 "ERROR : Possible infinite loop detected due to enter mutex timeout");
      return FALSE;
    case WAIT_ABANDONED:
...
BOOLEAN EnterMutexWithTimeout(UINT32 uiMutexIndex, UINT32 uiTimeout, INT32 nLine,
                              char *szFilename) {
