...
// Since all string input will have to be handle by reentrant capable functions (since we must
// attend to windows messaging as well as network traffic related issues), whenever there is ongoing
// string input going on, we must use InitStringInput() and HandleStringInput() to get the job done.
// HandleStringInput() will return TRUE as long as the string input is going on, and FALSE when its
// done
//
...
void EndStringInput(
    StringInput *pStringDescriptor) {  // Make sure we have a valid pStringDescriptor
  if (pStringDescriptor != NULL) {  // make sure the gpCurrentStringDescriptor is NULL if necessary
    if (pStringDescriptor == gpCurrentStringDescriptor) {
      gpCurrentStringDescriptor = NULL;
      gfCurrentStringInputState = FALSE;
    }
    // Make sure we have a valid string within the string descriptor
    if (pStringDescriptor->pOriginalString != NULL) {  // free up the string
