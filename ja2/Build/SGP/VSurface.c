...
BOOLEAN BltVSurfaceUsingDDBlt(HVSURFACE hDestVSurface, HVSURFACE hSrcVSurface, UINT32 fBltFlags,
                              INT32 iDestX, INT32 iDestY, RECT *SrcRect, RECT *DestRect);
...
BOOLEAN SetVideoSurfacePalette(HVSURFACE hVSurface, SGPPaletteEntry *pSrcPalette) {
  ...
  if (hVSurface->pPalette == NULL) {
    ...
    // Set into surface
    // DDSetSurfacePalette( (LPDIRECTDRAWSURFACE2)hVSurface->pSurfaceData,
    // (LPDIRECTDRAWPALETTE)hVSurface->pPalette );
...
BOOLEAN AddVSurfaceRegions(HVSURFACE hVSurface, VSURFACE_REGION **ppNewRegions,
                           UINT16 uiNumRegions) {
  UINT16 cnt;
...
// For testing with non-DDraw blitting, uncomment to test -- DB
    ...
    // Blt8BPPDataTo8BPPBuffer( UINT8 *pBuffer, UINT32 uiDestPitchBYTES, HVOBJECT hSrcVObject, INT32
    // iX, INT32 iY, UINT16 usIndex );
...
HVSURFACE GetFrameBufferVideoSurface() { return (ghFrameBuffer); }

HVSURFACE GetMouseBufferVideoSurface() { return (ghMouseBuffer); }
