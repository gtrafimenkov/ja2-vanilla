...
//							abound (and there were already bugs in
// do_copy() for
// ordered lists) 							While the memcpy() was
// working in _DEBUG mode, it was failing
// reproducibly in RELEASE mode whenever regions overlapped! Having read the code, I strongly
// suggest that you DO NOT use this stuff at all and write your own instead.  Tarun was no
// Carmack...
//																									-
// Alex Meduna
...
HORDLIST CreateOrdList(UINT32 uiNum_items, UINT32 uiSiz_each,
  ...
  if ((uiNum_items > 0) && (uiSiz_each > 0))
    ...
    DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
               "Requested ordered list items and size have to be >0");
    return 0;
  }
...
BOOLEAN DeleteOrdList(HORDLIST hOrdList) {
  if (hOrdList == NULL) {
    DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
               "This is not a valid pointer to the ordered list");
    return FALSE;
  }
  // free the memory assigned to the list
  MemFree(hOrdList);
...
BOOLEAN PeekQueue(HQUEUE hQueue, void *pdata) {
  ...
  if (pdata == NULL) {
    DbgMessage(TOPIC_QUEUE_CONTAINERS, DBG_LEVEL_0,
               "Memory fo Data to be removed from queue is NULL");
    return FALSE;
  }
...
//								Note: The data being swapped in MUST
// have the same
// size as 								the current node size or
// strange things
// could happen. 								Contents of node and
// pdata parameter are swapped.
//
// Parameter List : hList - pointer to list container
//									pdata - pointer to data to
// be
// swapped 									uiPos - List
...
//									Unlike SwapListNode(), this
// does NOT swap
// previous contents 									back into
// the pdata buffer!
//
// Parameter List : hList - pointer to list container
//									pdata - pointer to data to
...
BOOLEAN PeekOrdList(HORDLIST hOrdList, void *pdata, UINT32 uiPos) {
  ...
  if (pdata == NULL) {
    DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
               "Memory fo Data to be removed from ordered list is NULL");
    return FALSE;
  }
...
BOOLEAN RemfromQueue(HQUEUE hQueue, void *pdata) {
  ...
  if (pTemp_cont->uiHead == pTemp_cont->uiMax_size) pTemp_cont->uiHead = sizeof(QueueHeader);
...
HQUEUE AddtoQueue(HQUEUE hQueue, void *pdata) {
  ...
  if ((uiHead == uiTail) &&
      ((uiHead >= (sizeof(QueueHeader) + uiSize_of_each)) || (fresize == TRUE))) {
...
HLIST AddtoList(HLIST hList, void *pdata, UINT32 uiPos) {
  ...
  if (uiTail == uiOffsetSrc) fTail_check = TRUE;
  ...
  if (((uiTail + uiSize_of_each) <= uiMax_size) &&
    ...
    if (fTail_check == FALSE) pTemp_cont->uiTail += uiSize_of_each;
  ...
  memmove(pvoid, pdata, pTemp_cont->uiSiz_of_elem);
  pTemp_cont->uiTotal_items++;
  if (fTail_check == TRUE) pTemp_cont->uiTail += pTemp_cont->uiSiz_of_elem;
  return hList;
}
...
BOOLEAN RemfromList(HLIST hList, void *pdata, UINT32 uiPos) {
  ...
  if ((uiTail < uiHead) ||
      ((uiTail == uiHead) && (uiHead <= (sizeof(ListHeader) + uiSize_of_each)))) {
    uiOffsetSrc = pTemp_cont->uiHead + (uiPos * pTemp_cont->uiSiz_of_elem);
...
BOOLEAN RemfromOrdList(HORDLIST hOrdList, void *pdata, UINT32 uiPos) {
  ...
  if (hOrdList == NULL) {
    DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
               "This is not a valid handle to the ordered list");
    return FALSE;
  }
  ...
  if (pdata == NULL) {
    DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
               "Data to be pushed onto ordered list is NULL");
    return FALSE;
  }
  // check for a 0 or negative position passed in
  if (uiPos < 0) {
  ...
  if (pTemp_cont->uiTotal_items == 0) {
    DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
               "There are no elements in the ordered list to remove");
    return FALSE;
  }
  ...
  if ((uiTail > uiHead) || ((uiTail == uiHead) && (uiHead == sizeof(OrdListHeader)))) {
    ...
    if (do_copy_data(hOrdList, pdata, uiOffsetSrc, uiSize_of_each) == FALSE) {
      DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
                 "Could not copy the data from ordered list");
      return FALSE;
    }

    if (do_copy(hOrdList, uiOffsetDst, uiOffsetSrc, uiTail - uiOffsetSrc) == FALSE) {
      DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
                 "Could not remove the data from the ordered list");
      return FALSE;
    }
  ...
  if ((uiTail < uiHead) ||
      ((uiTail == uiHead) && (uiHead <= (sizeof(OrdListHeader) + uiSize_of_each)))) {
    uiOffsetSrc = pTemp_cont->uiHead + (uiPos * pTemp_cont->uiSiz_of_elem);
    ...
    if (uiOffsetSrc >= uiMax_size) {
      ...
      if (do_copy(hOrdList, uiOffsetSrc, uiOffsetDst,
...
HORDLIST StoreinOrdList(HORDLIST hOrdList, void *pdata, UINT32 uiPos) {
  ...
  if (uiPos > pTemp_cont->uiTotal_items) {
    DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
               "There are not enough elements in the ordered list to add after");
    return NULL;
  }
  ...
  if (uiOffsetSrc >= uiMax_size) uiOffsetSrc = sizeof(OrdListHeader) + (uiOffsetSrc - uiMax_size);
  ...
  if (((uiTail + uiSize_of_each) <= uiMax_size) &&
    ...
    if (fTail_check == FALSE) {
      if (do_copy(hOrdList, uiOffsetSrc, uiOffsetDst, uiTail - uiOffsetSrc) == FALSE) {
        DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
                   "Could not store the data in ordered list");
        return NULL;
      }
    }
  ...
  if ((((uiTail + uiSize_of_each) <= uiMax_size) && (uiTail < uiHead)) ||
      (((uiTail + uiSize_of_each) > uiMax_size) &&
       (uiHead >= (sizeof(OrdListHeader) + uiSize_of_each)))) {
    uiOffsetSrc = pTemp_cont->uiHead + (uiPos * pTemp_cont->uiSiz_of_elem);
  ...
  if ((((uiTail + uiSize_of_each) <= uiMax_size) && (uiTail == uiHead) &&
    ...
    if ((hOrdList = MemRealloc(hOrdList, uiNew_size)) == NULL) {
      DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
                 "Could not resize ordered list container memory");
      return NULL;
    }

    pTemp_cont = (OrdListHeader *)hOrdList;

    if (do_copy(hOrdList, sizeof(OrdListHeader), uiMax_size, uiHead - sizeof(OrdListHeader)) ==
        FALSE) {
      DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
                 "Could not copy ordered list container memory");
      return NULL;
    }
...
HORDLIST AddtoOrdList(HORDLIST hOrdList, void *pdata) {
  ...
  if (pOrdList->uiHead == pOrdList->uiTail) {
    ...
    if (pOrdList->uiHead == sizeof(OrdListHeader)) {
      ...
      if ((hOrdList = StoreinOrdList(hOrdList, pdata, 0)) == FALSE) {
        DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
                   "Could not copy the data into ordered list");
        return NULL;
      }
  ...
  // continue looping while offset hasn't reached the start of the list yet (or at least once if
  // list is full)
  while ((uiOffset != pOrdList->uiHead) || (fContinue)) {
    ...
    if (do_copy_data(hOrdList, pTemp_data, (uiOffset - pOrdList->uiSiz_of_elem),
                     pOrdList->uiSiz_of_elem) == FALSE) {
      DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0, "Could not copy the data from list");
      MemFree(pTemp_data);
      return NULL;
    }
    ...
    switch (sbResult) {
      case ORDLIST_ERROR: {
        DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
                   "Could not perform comparison for ordered lists");
        MemFree(pTemp_data);
        return NULL;
      }
      ...
      default: {
        DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
                   "Invalid result received from Compare function");
        MemFree(pTemp_data);
        return NULL;
      }
    }  // end switch
  ...
  if (uiOffset != pOrdList->uiHead) {
    DbgMessage(TOPIC_ORDLIST_CONTAINERS, DBG_LEVEL_0,
               "ERROR - left the while loop but not at the head");
    return NULL;
  }
...
INT8 Compare(void *p, void *q, UINT32 size) {
  ...
  if (temp1->me < temp2->me) return ORDLIST_LEFT_LESS;

  if (temp1->me > temp2->me) return ORDLIST_RIGHT_LESS;

  if (temp1->me == temp2->me) return ORDLIST_EQUAL;
