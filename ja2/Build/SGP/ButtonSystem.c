    ...
    if (b && b->Area.uiFlags & MSYS_REGION_ENABLED) {
      ...
      if (b->uiFlags & BUTTON_DIRTY) {
        ...
        InvalidateRegion(b->Area.RegionTopLeftX, b->Area.RegionTopLeftY, b->Area.RegionBottomRightX,
                         b->Area.RegionBottomRightY);
//#else
//				InvalidateRegion(b->Area.RegionTopLeftX, b->Area.RegionTopLeftY,
// b->Area.RegionBottomRightX, b->Area.RegionBottomRightY, FALSE);
#endif
      }
    }
  }
...
void DrawGenericButton(GUI_BUTTON *b) {
  ...
  for (q = 0; q < NumChunksWide; q++) {
    ...
    if (gbPixelDepth == 16) {
      ...
      Blt8BPPDataTo8BPPBufferTransparentClip((UINT16 *)pDestBuf, uiDestPitchBYTES, BPic,
                                             (INT32)(b->XLoc + (q * iBorderWidth)), (INT32)b->YLoc,
                                             (UINT16)ImgNum, &ClipRect);
    }
  ...
  if (gbPixelDepth == 16) {
    ...
    Blt8BPPDataTo8BPPBufferTransparentClip((UINT16 *)pDestBuf, uiDestPitchBYTES, BPic, cx,
                                           (INT32)b->YLoc, 2, &ClipRect);
  }
  ...
  if (gbPixelDepth == 16) {
    ...
    Blt8BPPDataTo8BPPBufferTransparentClip((UINT16 *)pDestBuf, uiDestPitchBYTES, BPic, cx, cy, 7,
                                           &ClipRect);
  }
  ...
  for (q = 1; q < NumChunksHigh; q++) {
    if (gbPixelDepth == 16) {
      Blt8BPPDataTo16BPPBufferTransparentClip((UINT16 *)pDestBuf, uiDestPitchBYTES, BPic,
                                              (INT32)b->XLoc,
                                              (INT32)(b->YLoc + (q * iBorderHeight)), 3, &ClipRect);
    } else if (gbPixelDepth == 8) {
      Blt8BPPDataTo8BPPBufferTransparentClip((UINT16 *)pDestBuf, uiDestPitchBYTES, BPic,
...
BOOLEAN SetDialogAttributes(CreateDlgInfo *pDlgInfo, INT32 iAttrib, ...) {
  ...
  switch (iAttrib) {
    ...
    case DLG_TEXT:
      ...
      if (pDlgInfo->zDlgText != NULL) MemFree(pDlgInfo->zDlgText);
...
INT32 CreateDialogBox(CreateDlgInfo *pDlgInfo) { return (-1); }
