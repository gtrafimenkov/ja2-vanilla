...
void MemDebug(BOOLEAN f) { gfMemDebug = f; }
...
PTR MemAllocReal(UINT32 uiSize, const char *pcFile, INT32 iLine) {
  ...
  if (!fMemManagerInit) {
    DbgMessage(TOPIC_MEMORY_MANAGER, DBG_LEVEL_0,
               String("MemAlloc: Warning -- Memory manager not initialized -- Line %d in %s", iLine,
                      pcFile));
  }
...
void MemFreeReal(PTR ptr, const char *pcFile, INT32 iLine) {
  ...
  if (!fMemManagerInit) {
    DbgMessage(TOPIC_MEMORY_MANAGER, DBG_LEVEL_0,
               String("MemFree: Warning -- Memory manager not initialized -- Line %d in %s", iLine,
                      pcFile));
  }
  ...
  // count even a NULL ptr as a MemFree, not because it's really a memory leak, but because it is
  // still an error of some sort (nobody should ever be freeing NULL pointers), and this will help
  // in tracking it down if the above DbgMessage is not noticed.
  MemDebugCounter--;
}
...
PTR MemReallocReal(PTR ptr, UINT32 uiSize, const char *pcFile, INT32 iLine) {
  ...
  if (ptrNew == NULL) {
    DbgMessage(TOPIC_MEMORY_MANAGER, DBG_LEVEL_0,
               String("MemReAlloc failed: ptr %d, %d -> %d bytes (line %d file %s)", ptr, uiOldSize,
                      uiSize, iLine, pcFile));
    if (uiSize != 0) {
...
PTR MemAllocLocked(UINT32 uiSize) {
  ...
  if (!fMemManagerInit) {
    DbgMessage(TOPIC_MEMORY_MANAGER, DBG_LEVEL_0,
               String("MemAllocLocked: Warning -- Memory manager not initialized!!! "));
  }
  ...
  if (ptr) {
    ...
    DbgMessage(TOPIC_MEMORY_MANAGER, DBG_LEVEL_0,
               String("MemAllocLocked failed: %d bytes", uiSize));
  }
...
void MemFreeLocked(PTR ptr, UINT32 uiSize) {
  if (!fMemManagerInit) {
    DbgMessage(TOPIC_MEMORY_MANAGER, DBG_LEVEL_0,
               String("MemFreeLocked: Warning -- Memory manager not initialized!!! "));
  }
  ...
  if (ptr != NULL) {
    ...
    DbgMessage(TOPIC_MEMORY_MANAGER, DBG_LEVEL_0,
               String("MemFreeLocked ERROR: NULL ptr received, size %d", uiSize));
  }
...
PTR MemReallocXDebug(PTR ptr, UINT32 size, const char *szCodeString, INT32 iLineNum,
                     void *pSpecial) {
  MEMORY_NODE *curr;
  PTR ptrNew;
  UINT16 usLength;
