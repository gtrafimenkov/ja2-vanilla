...
// setting file
// attributes which 									   allows
// for read-only attribute overriding
//									-> Also added a simple
// function that clears all file attributes to normal.
//
...
void ShutdownFileManager(void) { UnRegisterDebugTopic(TOPIC_FILE_MANAGER, "File Manager"); }
...
BOOLEAN FileDelete(STR strFilename) { return (DeleteFile(strFilename)); }
...
//
//		24sep96:HJH		-> creation
//		08Dec97:ARM		-> return FALSE if bytes to read != bytes read (CHECKF is
// inappropriate?)
//
//**************************************************************************
...
BOOLEAN FileLoad(STR strFilename, PTR pDest, UINT32 uiBytesToRead, UINT32 *puiBytesRead) {
  ...
  if (hFile) {
    ...
    if (uiBytesToRead != uiNumBytesRead) fRet = FALSE;

    if (puiBytesRead) *puiBytesRead = uiNumBytesRead;
...
BOOLEAN FileSeek(HWFILE hFile, UINT32 uiDistance, UINT8 uiHow) {
  ...
  if (sLibraryID == REAL_FILE_LIBRARY_ID) {
    ...
    if (SetFilePointer(hRealFile, iDistance, NULL, dwMoveMethod) == 0xFFFFFFFF) return (FALSE);
  } else {
    // if the database is initialized
...
HWFILE CreateFileHandle( HANDLE hRealFile, BOOLEAN fDatabaseFile )
{
        ...
        pNewFileInfo = (FMFileInfo *)MemRealloc( gfs.pFileInfo, gfs.uiNumHandles +
NUM_FILES_TO_ADD_AT_A_TIME ); if ( !pNewFileInfo )
        {
...
void BuildFileDirectory(void) {
  ...
  /*
     ...
     cSubDir[FILENAME_LENGTH]; HCONTAINER			hStack;
          ...
          if (hStack == NULL)
          {
                  FastDebugMsg(String("BuildFileDirectory: CreateStack Failed for the filename
     stack")); return;
          }
          ...
          while ( fMore )
          {
                  ...
                  {
                          if ( strcmp( find.cFileName, "." ) != 0 && strcmp( find.cFileName, ".." )
     != 0 )
                          {
                                  // a valid directory
                                  inFind.dwFileAttributes = FILE_ATTRIBUTE_NORMAL |
     FILE_ATTRIBUTE_DIRECTORY; strcpy( cSubDir, cDir ); strcpy( &(cSubDir[strlen(cDir)-3]),
     find.cFileName ); strcpy( &(cSubDir[strlen(cSubDir)]), "\\*.*\0" ); hFileIn = FindFirstFile(
     cSubDir, &inFind ); iNumFiles += GetFilesInDirectory( hStack, cSubDir, hFileIn, &inFind );
                                  FindClose( hFileIn );
                          }
                  }
                  else
...
BOOLEAN SetFileManCurrentDirectory(STR pcDirectory) { return (SetCurrentDirectory(pcDirectory)); }
...
BOOLEAN DirectoryExists(STRING512 pcDirectory) {
  ...
  if (uiAttribs == 0xFFFFFFFF) {
    ...
    if (uiLastError != ERROR_FILE_NOT_FOUND) {
      FastDebugMsg(String("DirectoryExists: ERROR - GetFileAttributes failed, error #%d on file %s",
...
BOOLEAN MakeFileManDirectory(STRING512 pcDirectory) { return CreateDirectory(pcDirectory, NULL); }

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Removes ALL FILES in the specified directory (and all subdirectories with their files if
// fRecursive is TRUE) Use EraseDirectory() to simply delete directory contents without deleting the
// directory itself
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOLEAN RemoveFileManDirectory(STRING512 pcDirectory, BOOLEAN fRecursive) {
  ...
  if (!SetFileManCurrentDirectory(pcDirectory)) {
    FastDebugMsg(
        String("RemoveFileManDirectory: ERROR - SetFileManCurrentDirectory on %s failed, error %d",
        ...
        if (fRecursive) {
          ...
          if ((strcmp(sFindData.cFileName, ".") != 0) && (strcmp(sFindData.cFileName, "..") != 0)) {
            if (!RemoveFileManDirectory(zSubdirectory, TRUE)) {
              FastDebugMsg(String("RemoveFileManDirectory: ERROR - Recursive call on %s failed",
                                  zSubdirectory));
              break;
            }
          }
  ...
  if (!fRetval) {
    FastDebugMsg(String("RemoveFileManDirectory: ERROR - RemoveDirectory on %s failed, error %d",
                        pcDirectory, GetLastError()));
  }
...
BOOLEAN EraseDirectory(STRING512 pcDirectory) {
  ...
  if (!SetFileManCurrentDirectory(pcDirectory)) {
    FastDebugMsg(String("EraseDirectory: ERROR - SetFileManCurrentDirectory on %s failed, error %d",
                        pcDirectory, GetLastError()));
    return (FALSE);  // Error going into directory
  }
  ...
  /*
    ...
    if (hFile == 0)
    {
      ...
      return(FALSE);
          ...
          if (pBuffer == NULL)
          {
                  FastDebugMsg(String("FileCopy: ERROR - MemAlloc pBuffer failed, size %d",
    uiSize)); FileClose(hFile); return(FALSE);
          }
...
BOOLEAN FileSetAttributes(STR strFilename, UINT32 uiNewAttribs) {
  ...
  if (uiNewAttribs & FILE_ATTRIBUTES_ARCHIVE) uiFileAttrib |= FILE_ATTRIBUTE_ARCHIVE;

  if (uiNewAttribs & FILE_ATTRIBUTES_HIDDEN) uiFileAttrib |= FILE_ATTRIBUTE_HIDDEN;

  if (uiNewAttribs & FILE_ATTRIBUTES_NORMAL) uiFileAttrib |= FILE_ATTRIBUTE_NORMAL;

  if (uiNewAttribs & FILE_ATTRIBUTES_OFFLINE) uiFileAttrib |= FILE_ATTRIBUTE_OFFLINE;

  if (uiNewAttribs & FILE_ATTRIBUTES_READONLY) uiFileAttrib |= FILE_ATTRIBUTE_READONLY;

  if (uiNewAttribs & FILE_ATTRIBUTES_SYSTEM) uiFileAttrib |= FILE_ATTRIBUTE_SYSTEM;

  if (uiNewAttribs & FILE_ATTRIBUTES_TEMPORARY) uiFileAttrib |= FILE_ATTRIBUTE_TEMPORARY;
...
BOOLEAN FileCheckEndOfFile(HWFILE hFile) {
  ...
  if (sLibraryID == REAL_FILE_LIBRARY_ID) {
    ...
    temp =
        SetFilePointer(hRealFile, -((INT32)(uiEndOfFilePtrLoc - uiOldFilePtrLoc)), NULL, FILE_END);
...
BOOLEAN AddSubdirectoryToPath(CHAR8 *pDirectory) {
  ...
  if (!strlen(pDirectory)) return (FALSE);

  if ((pSystemPath = (CHAR8 *)MemAlloc(_MAX_PATH)) == NULL) return (FALSE);
  ...
  if (uiPathLen) uiPathLen--;
  if (pPath[uiPathLen] != '\\') strcat(pPath, "\\");
