...
STR GetExceptionString(DWORD uiExceptionCode) {
  switch (uiExceptionCode) {
    ...
    case EXCEPTION_ARRAY_BOUNDS_EXCEEDED:
      return (
          "The thread tried to access an array element that is out of bounds and the underlying "
    ...
    case EXCEPTION_IN_PAGE_ERROR:
      return (
          ...
          "load the page. For example, this exception might occur if a network connection is lost "
    ...
    case EXCEPTION_INVALID_DISPOSITION:
      return (
          ...
          "Programmers using a high-level language such as C should never encounter this "
          "exception.");
    ...
    case EXCEPTION_SINGLE_STEP:
      return (
          "A trace trap or other single-instruction mechanism signaled that one instruction has "
          "been executed.");
...
//
// This code for this function is based ( stolen ) from Bruce Dawson's article in Game Developer
// Magazine Jan 99
//
BOOLEAN DisplayStack(HWFILE hFile, CONTEXT *pContext) {
  int Count = 0;
  ...
  __try {
    ...
    while (pStack + 1 <= pStackTop) {
      ...
      if ((Count % StackColumns) == 0) output += wsprintf(output, "%08x: ", pStack);

      if ((++Count % StackColumns) == 0 || pStack + 2 > pStackTop) Suffix = "\r\n";
...
//
// This code for this function is ( stolen :) from Bruce Dawson's article in Game Developer Magazine
// Jan 99
//
...
void PrintTime(char *output, FILETIME TimeToPrint) {
  ...
  if (FileTimeToLocalFileTime(&TimeToPrint, &TimeToPrint) &&
    ...
    wsprintf(output, "%02d/%02d/%d %02d:%02d:%02d", (Date / 32) & 15, Date & 31,
             (Date / 512) + 1980, (Time / 2048), (Time / 32) & 63, (Time & 31) * 2);
  } else
    output[0] = 0;
}
...
static void ShowModuleInfo(HWFILE hFile, HINSTANCE ModuleHandle) {
  ...
  __try {
    if (GetModuleFileName(ModuleHandle, ModName, sizeof(ModName)) > 0) {
      ...
      if (IMAGE_NT_SIGNATURE != NTHeader->Signature) return;
