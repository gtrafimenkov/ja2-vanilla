...
// DirectDrawSurface2 Calls
void DDCreateSurfaceInMemory(LPDIRECTDRAW2 pExistingDirectDraw, DDSURFACEDESC *pNewSurfaceDesc,
  ...
  fDestination = (DDSurfaceDesc.ddsCaps.dwCaps & DDSCAPS_VIDEOMEMORY) ? TRUE : FALSE;
...
void DDCreateRasterSurface(LPDIRECTDRAW2 pDirectDraw, INT32 iWidth, INT32 iHeight,
  ...
  DDCreateSurfaceInMemory(pDirectDraw, &DDSurfaceDesc, fVideoMemory, ppRasterSurface1,
                          ppRasterSurface2);
}
...
void DDCreateZBufferSurface(LPDIRECTDRAW2 pDirectDraw, INT32 iWidth, INT32 iHeight,
  ...
  DDCreateSurfaceInMemory(pDirectDraw, &DDSurfaceDesc, fVideoMemory, ppZBufferSurface1,
                          ppZBufferSurface2);
}
...
void DDAddAttachedSurface(LPDIRECTDRAWSURFACE2 pParentSurface,
  ...
  ATTEMPT(IDirectDrawSurface2_AddAttachedSurface(pParentSurface, pAddChildSurface));
}
...
void DDDeleteAttachedSurface(LPDIRECTDRAWSURFACE2 pParentSurface,
  ...
  ATTEMPT(IDirectDrawSurface2_DeleteAttachedSurface(pParentSurface, 0, pDeleteChildSurface));
}
...
void DDGetDDInterface(LPDIRECTDRAWSURFACE2 pSurface, LPDIRECTDRAW *ppDirectDraw) {
  ...
  ATTEMPT(
      IDirectDrawSurface2_GetDDInterface((IDirectDrawSurface2 *)pSurface, (LPVOID *)ppDirectDraw));
}
...
HRESULT BltFastDDSurfaceUsingSoftware(LPDIRECTDRAWSURFACE2 pDestSurface, INT32 uiX, INT32 uiY,
                                      LPDIRECTDRAWSURFACE2 pSrcSurface, LPRECT pSrcRect,
                                      UINT32 uiTrans) {
  ...
  if (uiTrans == DDBLTFAST_NOCOLORKEY) {
    Blt16BPPTo16BPP((UINT16 *)pDestBuf, uiDestPitchBYTES, (UINT16 *)pSrcBuf, uiSrcPitchBYTES, uiX,
                    uiY, pSrcRect->left, pSrcRect->top, (pSrcRect->right - pSrcRect->left),
    ...
    if (ReturnCode == DD_OK) {
      ...
      Blt16BPPTo16BPPTrans((UINT16 *)pDestBuf, uiDestPitchBYTES, (UINT16 *)pSrcBuf, uiSrcPitchBYTES,
                           uiX, uiY, pSrcRect->left, pSrcRect->top,
...
HRESULT BltDDSurfaceUsingSoftware(LPDIRECTDRAWSURFACE2 pDestSurface, LPRECT pDestRect,
                                  LPDIRECTDRAWSURFACE2 pSrcSurface, LPRECT pSrcRect, UINT32 uiFlags,
                                  LPDDBLTFX pDDBltFx) {
  DDSURFACEDESC SurfaceDescription;
  UINT32 uiDestPitchBYTES, uiSrcPitchBYTES;
  ...
  if (pSrcRect != NULL &&
    ...
    Blt16BPPTo16BPP((UINT16 *)pDestBuf, uiDestPitchBYTES, (UINT16 *)pSrcBuf, uiSrcPitchBYTES,
                    pDestRect->left, pDestRect->top, pSrcRect->left, pSrcRect->top,
                    (pSrcRect->right - pSrcRect->left), (pSrcRect->bottom - pSrcRect->top));
    ...
    // do color fill here...
    FillRect16BPP((UINT16 *)pDestBuf, uiDestPitchBYTES, pDestRect->left, pDestRect->top,
                  pDestRect->right, pDestRect->bottom, (UINT16)pDDBltFx->dwFillColor);
  } else {
    // Not supported.....
  }
