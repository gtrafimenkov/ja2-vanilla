...
BOOLEAN CopyImageToBuffer(HIMAGE hImage, UINT32 fBufferType, BYTE *pDestBuf, UINT16 usDestWidth,
  ...
  if (hImage->ubBitDepth == 8 && fBufferType == BUFFER_8BPP) {
    ...
    if (hImage->fFlags & IMAGE_COMPRESSED) {
      ...
      return (Copy8BPPCompressedImageTo8BPPBuffer(hImage, pDestBuf, usDestWidth, usDestHeight, usX,
                                                  usY, srcRect));
    }
#endif
  ...
  if (hImage->ubBitDepth == 8 && fBufferType == BUFFER_16BPP) {
    ...
    if (hImage->fFlags & IMAGE_COMPRESSED) {
      ...
      return (Copy8BPPCompressedImageTo16BPPBuffer(hImage, pDestBuf, usDestWidth, usDestHeight, usX,
                                                   usY, srcRect));
    }
#endif
  ...
  if (hImage->ubBitDepth == 16 && fBufferType == BUFFER_16BPP) {
    ...
    if (hImage->fFlags & IMAGE_COMPRESSED) {
      ...
      return (Copy16BPPCompressedImageTo16BPPBuffer(hImage, pDestBuf, usDestWidth, usDestHeight,
                                                    usX, usY, srcRect));
    }
#endif
