...
void MakeClosestEnemyChosenOne() {
  ...
  for (cnt = 0; cnt < guiNumMercSlots; cnt++) {
    ...
    /*
    if ( FindAdjacentGridEx( pSoldier, gTacticalStatus.sPanicTriggerGridno, &ubDirection, &sAdjSpot,
    FALSE, FALSE ) == -1 )
    {
    ...
    */
    ...
    if (pSoldier->sGridNo != sPanicTriggerGridNo) {
      ...
      sPathCost =
          PlotPath(pSoldier, sPanicTriggerGridNo, FALSE, FALSE, FALSE, WALKING, FALSE, FALSE, 0);
    } else {
      sPathCost = 0;
    }
...
void PossiblyMakeThisEnemyChosenOne(SOLDIERCLASS *pSoldier) {
  ...
  if (pSoldier->sGridNo != sPanicTriggerGridNo) {
    iPathCost =
        PlotPath(pSoldier, sPanicTriggerGridNo, FALSE, FALSE, FALSE, RUNNING, FALSE, FALSE, 0);
    if (iPathCost == 0) {
      // pSoldier->bHasKeys = bOldKeys;
      pSoldier->bHasKeys = (pSoldier->bHasKeys >> 1);
      return;
