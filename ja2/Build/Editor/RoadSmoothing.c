...
// the second contains the gridno offset from the anchor position (where the user clicks in the
// world to place the road).  The actual index of the array refers to the offset from ROADPIECE001.
...
// Road macros vary in size from 3 gridnos to 18 gridnos.  Using the anchor gridno based off of the
// original road system, this function will place the new macro (consisting of multiple road pieces
// in multiple gridnos).
...
// The old road system used multi-tiled roads as a single image.  The new road system has taken
// these large pieces and chopped them up into single tiled images (to mitigate lighting problems).
// Some of the larger road pieces turned into 18 smaller pieces.  So this function will go analyse
// the world, and replaces any locations containing the original road tile information, delete it,
// and replace it by inserting it's equivalent macro.
