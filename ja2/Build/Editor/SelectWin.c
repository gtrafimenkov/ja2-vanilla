...
void CreateJA2SelectionWindow(INT16 sWhat) {
  ...
  iSelectWin = CreateHotSpot(0, 0, H_OFFSET, /*360*/ giScrH - 120, MSYS_PRIORITY_HIGH,
                             DEFAULT_MOVE_CALLBACK, SelWinClkCallback);

  iCancelWin = CreateIconButton((INT16)iButtonIcons[CANCEL_ICON], 0, BUTTON_USE_DEFAULT, H_OFFSET,
                                40, 40, 40, BUTTON_TOGGLE, MSYS_PRIORITY_HIGH,
                                DEFAULT_MOVE_CALLBACK, CnclClkCallback);
  SetButtonFastHelpText(iCancelWin, L"Cancel selections");

  iOkWin =
      CreateIconButton((INT16)iButtonIcons[OK_ICON], 0, BUTTON_USE_DEFAULT, H_OFFSET, 0, 40, 40,
                       BUTTON_TOGGLE, MSYS_PRIORITY_HIGH, DEFAULT_MOVE_CALLBACK, OkClkCallback);
  SetButtonFastHelpText(iOkWin, L"Accept selections");

  iScrollUp =
      CreateIconButton((INT16)iButtonIcons[UP_ICON], 0, BUTTON_USE_DEFAULT, H_OFFSET, 80, 40, 160,
  ...
  iScrollDown = CreateIconButton((INT16)iButtonIcons[DOWN_ICON], 0, BUTTON_USE_DEFAULT, H_OFFSET,
                                 240, 40, 160, BUTTON_NO_TOGGLE, MSYS_PRIORITY_HIGH,
  ...
  SelWinEndPoint.iX = H_OFFSET - 1;  /// 599;
  SelWinEndPoint.iY = giScrH - 121;  /// 359;

  iBotWinCutOff = giScrH - 121;  /// 359;
...
void InitJA2SelectionWindow(void) {
  ...
  for (iCount = 0; iCount < (THIRDOSTRUCT - FIRSTOSTRUCT); iCount++) {
    ...
    OStructs1[iCount + 1].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTOSTRUCT + iCount]].hTileSurface;
  ...
  for (iCount = 0; iCount < (LASTDEBRIS - DEBRISROCKS + 1); iCount++) {
    ...
    DebrisList[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[DEBRISROCKS + iCount]].hTileSurface;
  ...
  for (iCount2 = 0; iCount2 < 2 /*(LASTSLANTROOF - FIRSTSLANTROOF + 1)*/; iCount2++, iCount++) {
    Room[iCount].ubType = DISPLAY_GRAPHIC;
    Room[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTSLANTROOF + iCount2]].hTileSurface;
    Room[iCount].usStart = 0;
    Room[iCount].usEnd = 0;
  ...
  for (iCount = 0, iCount2 = 0; iCount < (LASTWALL - FIRSTWALL + 1); iCount++, iCount2 += 2) {
    ...
    SingleWall[iCount2].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTWALL + iCount]].hTileSurface;
  ...
  for (iCount = 0, iCount2 = 0; iCount < (LASTDOOR - FIRSTDOOR + 1); iCount++, iCount2 += 5) {
    ...
    SingleDoor[iCount2].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTDOOR + iCount]].hTileSurface;
  ...
  for (iCount = 0; iCount < (LASTROOF - FIRSTROOF + 1); iCount++) {  // Flat roofs
    SingleRoof[iCount].ubType = DISPLAY_GRAPHIC;
    SingleRoof[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTROOF + iCount]].hTileSurface;
  ...
  for (iCount2 = 0; iCount2 < (LASTWALL - FIRSTWALL + 1); iCount2++, iCount++) {  // A-Frames
    SingleRoof[iCount].ubType = DISPLAY_GRAPHIC;
    SingleRoof[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTWALL + iCount2]].hTileSurface;
  ...
  for (iCount2 = 0; iCount2 < (SECONDONROOF - FIRSTONROOF + 1); iCount2++, iCount++) {  // On roofs
    SingleRoof[iCount].ubType = DISPLAY_GRAPHIC;
    SingleRoof[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTONROOF + iCount2]].hTileSurface;
    SingleRoof[iCount].usStart = DISPLAY_ALL_OBJECTS;
    SingleRoof[iCount].usEnd = 0;
  ...
  for (iCount = 0; iCount < (LASTDECORATIONS - FIRSTDECORATIONS + 1);
       iCount++) {  // Old obsolete wall decals, but should be replaced with multitiled decals such
                    // as banners, etc.
    SingleBrokenWall[iCount].ubType = DISPLAY_GRAPHIC;
    SingleBrokenWall[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTDECORATIONS + iCount]].hTileSurface;
  ...
  for (iCount = 0; iCount < (LASTISTRUCT - FIRSTISTRUCT + 1); iCount++) {
    ...
    SingleDecor[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTISTRUCT + iCount]].hTileSurface;
  ...
  for (iCount = 0; iCount < (LASTWALLDECAL - FIRSTWALLDECAL + 1); iCount++) {
    ...
    SingleDecal[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTWALLDECAL + iCount]].hTileSurface;
  ...
  for (iCount2 = 0; iCount2 < (EIGTHWALLDECAL - FIFTHWALLDECAL + 1); iCount++, iCount2++) {
    SingleDecal[iCount].ubType = DISPLAY_GRAPHIC;
    SingleDecal[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIFTHWALLDECAL + iCount2]].hTileSurface;
    SingleDecal[iCount].usStart = DISPLAY_ALL_OBJECTS;
    SingleDecal[iCount].usEnd = 0;
  ...
  for (iCount = 0; iCount < (LASTFLOOR - FIRSTFLOOR + 1); iCount++) {
    ...
    SingleFloor[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIRSTFLOOR + iCount]].hTileSurface;
  ...
  for (iCount = 0; iCount < (EIGHTISTRUCT - FIFTHISTRUCT + 1); iCount++) {
    ...
    SingleToilet[iCount].hVObject =
        gTileDatabase[gTileTypeStartIndex[FIFTHISTRUCT + iCount]].hTileSurface;
...
//
//	Unloads selection window button images and makes sure any display list that may remain in
// memory 	is removed.
//
...
void RenderSelectionWindow(void) {
  ...
  ColorFillVideoSurfaceArea(FRAME_BUFFER, 0, 0, /*600*/ giScrW, /*400*/ giScrH - 120,
  ...
  if (gfRenderSquareArea) {
    ...
    if ((abs(iStartClickX - button->Area.MouseXPos) > 9) ||
      ...
      iEX = min(iEX, H_OFFSET);
      iSY = max(SelWinStartPoint.iY, iSY);
      iEY = min(/*359*/ giScrH - 121, iEY);
  ...
  if (pNode) {
    ...
    if (!gTilesets[giCurrentTilesetID].TileSurfaceFilenames[pNode->uiObjIndx][0]) {
      mprintf(2, 2, L"%S[%d] is from default tileset %s (%S)",
              gTilesets[0].TileSurfaceFilenames[pNode->uiObjIndx], pNode->uiIndex,
              gTilesets[0].zName, gTileSurfaceName[pNode->uiObjIndx]);
    } else {
      mprintf(2, 2, L"File:  %S, subindex:  %d (%S)",
              gTilesets[giCurrentTilesetID].TileSurfaceFilenames[pNode->uiObjIndx], pNode->uiIndex,
...
BOOLEAN BuildDisplayWindow(DisplaySpec *pDisplaySpecs, UINT16 usNumSpecs,
  ...
  for (usSpecLoop = 0; usSpecLoop < usNumSpecs; usSpecLoop++) {
    ...
    if (pDisplaySpec->ubType == DISPLAY_GRAPHIC) {
      if (!pDisplaySpec->hVObject) return FALSE;
...
BOOLEAN DisplayWindowFunc(DisplayList *pNode, INT16 iTopCutOff, INT16 iBottomCutOff,
  ...
  if (pNode->iY < iTopCutOff) return (TRUE);
