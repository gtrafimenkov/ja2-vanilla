...
UINT32 InitScreenHandle(void) {
  ...
  if (ubCurrentScreen == 0) {
...
#ifdef _DEBUG
    mprintf(giOffsW + 10, giOffsH + 430, L"%s: %s (Debug %S)", pMessageStrings[MSG_VERSION],
            zVersionLabel, czVersionNumber);
#else
    mprintf(giOffsW + 10, giOffsH + 430, L"%s: %s (%S)", pMessageStrings[MSG_VERSION],
            zVersionLabel, czVersionNumber);
#endif
    ...
    mprintf(giOffsW + 10, giOffsH + 420, L"(Beta version error reporting enabled)");

#endif

#ifdef _DEBUG
    mprintf(giOffsW + 10, giOffsH + 440, L"SOLDIERCLASS: %d bytes", sizeof(SOLDIERCLASS));
#endif

    if (gfDontUseDDBlits) {
#ifdef _DEBUG
      mprintf(giOffsW + 10, giOffsH + 450, L"SOLDIERCLASS: %d bytes", sizeof(SOLDIERCLASS));
#else
      mprintf(giOffsW + 10, giOffsH + 440, L"Using software blitters");
#endif
    }
...
UINT32 SexScreenHandle(void) {
  ...
  sX = (INT16)((giScrW - pTrav->usWidth) / 2);
  sY = (INT16)((giScrH - pTrav->usHeight) / 2);
...
void DoDemoIntroduction() {
  ...
  yp = 50;
  height = DisplayWrappedString(20, yp, 400, 3, FONT14ARIAL, FONT_YELLOW, gpDemoIntroString[0],
                                FONT_BLACK, FALSE, LEFT_JUSTIFIED);
  yp = (UINT16)(yp + 25 + height);
  SetFontForeground(FONT_GRAY2);
  mprintf(35, yp, L"-");
  height = DisplayWrappedString(45, yp, 385, 3, FONT14ARIAL, FONT_GRAY2, gpDemoIntroString[1],
                                FONT_BLACK, FALSE, LEFT_JUSTIFIED);
  yp = (UINT16)(yp + 25 + height);
  mprintf(35, yp, L"-");
  height = DisplayWrappedString(45, yp, 385, 3, FONT14ARIAL, FONT_GRAY2, gpDemoIntroString[2],
                                FONT_BLACK, FALSE, LEFT_JUSTIFIED);
  yp = (UINT16)(yp + 25 + height);
  mprintf(35, yp, L"-");
  height = DisplayWrappedString(45, yp, 385, 3, FONT14ARIAL, FONT_GRAY2, gpDemoIntroString[3],
                                FONT_BLACK, FALSE, LEFT_JUSTIFIED);
  yp = (UINT16)(yp + 30 + height);
  height = DisplayWrappedString(35, yp, 385, 3, FONT14ARIAL, FONT_GRAY3, gpDemoIntroString[4],
                                FONT_BLACK, FALSE, LEFT_JUSTIFIED);
  yp = (UINT16)(yp + 35 + height);
  height = DisplayWrappedString(20, yp, 400, 3, FONT14ARIAL, FONT_LTKHAKI, gpDemoIntroString[5],
                                FONT_BLACK, FALSE, LEFT_JUSTIFIED);
...
UINT32 DemoExitScreenHandle(void) {
  ...
  if (ubCurrentScreen == 0) {
    ...
    SetCurrentCursorFromDatabase(VIDEO_NO_CURSOR);
    MSYS_DefineRegion(&BackRegion, 0, 0, giScrW, giScrH, MSYS_PRIORITY_HIGHEST, VIDEO_NO_CURSOR,
  ...
  if (!uiStartTime) {
    uiStartTime = uiTime;
    usCenter = (UINT16)(320 - (StringPixLength(gpDemoString[0], FONT14ARIAL) + 10) +
                        (StringPixLength(gpDemoString[0], FONT14ARIAL) + 10));
  ...
  if (ubCurrentScreen == 3) {
    ...
    if (iPercentage < 50) {  // do the logo entrance section
      ...
      BltStretchVideoSurface(FRAME_BUFFER, uiCollageID, 0, 0, VO_BLT_SRCTRANSPARENCY, &SrcRect,
                             &DstRect);
      InvalidateScreen();
    } else {  // scale down the logo and stamp it.
      SrcRect.iLeft = 0;
      SrcRect.iRight = 263;
      SrcRect.iTop = 0;
      SrcRect.iBottom = 210;
      DstRect.iLeft = 189 * (iPercentage - 50) / 50;
      DstRect.iRight = giScrW - 188 * (iPercentage - 50) / 50;
      DstRect.iTop = 20 * (iPercentage - 50) / 50;
      DstRect.iBottom = giScrH - 250 * (iPercentage - 50) / 50;
      BlitBufferToBuffer(guiSAVEBUFFER, FRAME_BUFFER, 0, 0, giScrW, giScrH);
      BltStretchVideoSurface(FRAME_BUFFER, uiCollageID, 0, 0, VO_BLT_SRCTRANSPARENCY, &SrcRect,
                             &DstRect);
      InvalidateScreen();
    ...
    if (iPercentage == 100) {
      ...
      BlitBufferToBuffer(FRAME_BUFFER, guiSAVEBUFFER, 0, 0, giScrW, giScrH);
      ubCurrentScreen = 4;
      SetFontDestBuffer(FRAME_BUFFER, 0, 230, giScrW, giScrH, FALSE);
      SetFont(FONT10ARIAL);
      SetFontForeground(FONT_GRAY2);
      uiStartTime = GetJA2Clock();
      DeleteVideoSurfaceFromIndex(uiCollageID);
  ...
  if (ubCurrentScreen == 4) {
    ...
    yp = giScrH - iPercentage / 40;  // 500 (0%) to -500 (100%)
    yp = max(yp, -400);
    for (i = 2; i < 40; i++) {
      if (i < 36)
  ...
  if (ubCurrentScreen == 5) {
    ...
    if (iPercentage < 50) {  // do the logo entrance section
      ...
      DstRect.iRight = giScrW - 154 * (iPercentage - 50) / 50;
      DstRect.iTop = 246 * (iPercentage - 50) / 50;
      DstRect.iBottom = giScrH - 86 * (iPercentage - 50) / 50;
#ifdef GERMAN
  ...
  if (ubCurrentScreen == 7) {
    if (ubPreviousScreen == 6) {
      uiStartTime = GetJA2Clock();
#ifdef GERMAN
      uiStartTime = 4000000000;  // you could break the system by playing the game for 46 real-time
                                 // days or longer.
#endif
    }
    uiTime = GetJA2Clock();
