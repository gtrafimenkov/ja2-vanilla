...
SNDOBJ *SndObjCreate(IDirectSound *pDS, LPCTSTR lpName, int iConcurrent) {
  ...
  if (DSGetWaveResource(NULL, lpName, &pWaveHeader, &pbData, &cbData)) {
    ...
    if ((pSO = (SNDOBJ *)LocalAlloc(
             LPTR, sizeof(SNDOBJ) + (iConcurrent - 1) * sizeof(IDirectSoundBuffer *))) != NULL) {
      int i;
...
BOOL DSFillSoundBuffer(IDirectSoundBuffer *pDSB, BYTE *pbWaveData, DWORD cbWaveSize) {
  if (pDSB && pbWaveData && cbWaveSize) {
    ...
    if (SUCCEEDED(
            IDirectSoundBuffer_Lock(pDSB, 0, cbWaveSize, &pMem1, &dwSize1, &pMem2, &dwSize2, 0))) {
    ...
    switch (dwType) {
      case mmioFOURCC('f', 'm', 't', ' '):
        if (ppWaveHeader && !*ppWaveHeader) {
          if (dwLength < sizeof(WAVEFORMAT)) goto exit;  // not a WAV
