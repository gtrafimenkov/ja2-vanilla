...
BOOLEAN GetMLGFilename(SGPFILENAME filename, UINT16 usMLGGraphicID) {
  if (gbLocale != ENGLISH_LANG) {
    CHAR8 zLanguage[64];
    sprintf(zLanguage, LocaleNames[gbLocale]);
    ...
    switch (usMLGGraphicID) {
      ...
      case MLG_DESKTOP:
        ...
        break;
      case MLG_FUNERALAD9:
        if (gbLocale == GERMAN_LANG)
          sprintf(filename, "%s\\FuneralAd_12_german.sti", zLanguage);
        else
      ...
      case MLG_STOREPLAQUE:
        sprintf(filename, "%s\\BobbyStorePlaque_%s.sti", zLanguage, zLanguage);
        break;
      case MLG_TITLETEXT:
...
BOOLEAN GetMLGFilename( SGPFILENAME filename, UINT16 usMLGGraphicID )
{
                ...
                switch( usMLGGraphicID )
                {
                        ...
                        case MLG_BOBBYRAYAD21:
                                sprintf( filename, "%s\\BobbyRayAd_21_%s.sti", zLanguage, zLanguage
); return TRUE; case MLG_BOBBYRAYLINK: sprintf( filename, "%s\\BobbyRayLink_%s.sti", zLanguage,
zLanguage ); return TRUE; case MLG_CLOSED: sprintf( filename, "%s\\Closed_%s.sti", zLanguage,
zLanguage ); return TRUE; case MLG_CONFIRMORDER: sprintf( filename, "%s\\ConfirmOrder_%s.sti",
zLanguage, zLanguage ); return TRUE; case MLG_DESKTOP: sprintf( filename, "%s\\desktop_%s.pcx",
zLanguage, zLanguage ); return TRUE; case MLG_FUNERALAD9: sprintf( filename,
"%s\\FuneralAd_9_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_GOLDPIECEBUTTONS: sprintf(
filename, "%s\\goldpiecebuttons_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_HISTORY:
                                sprintf( filename, "%s\\history_%s.sti", zLanguage, zLanguage );
                                return TRUE;
                        case MLG_INSURANCEAD10:
                                sprintf( filename, "%s\\insurancead_10_%s.sti", zLanguage, zLanguage
); return TRUE; case MLG_INSURANCELINK: sprintf( filename, "%s\\insurancelink_%s.sti", zLanguage,
zLanguage ); return TRUE; case MLG_INSURANCETITLE: sprintf( filename, "%s\\largetitle_%s.sti",
zLanguage, zLanguage ); return TRUE; case MLG_LARGEFLORISTSYMBOL: sprintf( filename,
"%s\\LargeSymbol_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_ORDERGRID: sprintf(
filename, "%s\\OrderGrid_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_SMALLFLORISTSYMBOL:
                                sprintf( filename, "%s\\SmallSymbol_%s.sti", zLanguage, zLanguage );
                                return TRUE;
                        case MLG_STATSBOX:
                                sprintf( filename, "%s\\StatsBox_%s.sti", zLanguage, zLanguage );
                        ...
                        case MLG_MCGILLICUTTYS:
                                sprintf( filename, "%s\\McGillicuttys_%s.sti", zLanguage, zLanguage
); return TRUE; case MLG_MORTUARY: sprintf( filename, "%s\\Mortuary_%s.sti", zLanguage, zLanguage );
                                return TRUE;
                        case MLG_MORTUARYLINK:
                                sprintf( filename, "%s\\MortuaryLink_%s.sti", zLanguage, zLanguage
); return TRUE; case MLG_PREBATTLEPANEL: sprintf( filename, "%s\\PreBattlePanel_%s.sti", zLanguage,
zLanguage ); return TRUE; case MLG_SMALLTITLE: sprintf( filename, "%s\\SmallTitle_%s.sti",
zLanguage, zLanguage ); return TRUE; case MLG_STOREPLAQUE: sprintf( filename,
"%s\\StorePlaque_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_TITLETEXT: sprintf(
filename, "%s\\titletext_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_TOALUMNI: sprintf(
filename, "%s\\ToAlumni_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_TOMUGSHOTS: sprintf(
filename, "%s\\ToMugShots_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_TOSTATS: sprintf(
filename, "%s\\ToStats_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_WARNING: sprintf(
filename, "%s\\Warning_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_YOURAD13: sprintf(
filename, "%s\\YourAd_13_%s.sti", zLanguage, zLanguage ); return TRUE; case MLG_OPTIONHEADER:
                                sprintf( filename, "%s\\optionscreenaddons_%s.sti", zLanguage,
zLanguage ); return TRUE; case MLG_LOADSAVEHEADER: sprintf( filename, "%s\\loadscreenaddons_%s.sti",
zLanguage, zLanguage ); return TRUE; case MLG_SPLASH: sprintf( filename, "%s\\splash_%s.sti",
zLanguage, zLanguage ); return TRUE; case MLG_IMPSYMBOL: sprintf( filename, "%s\\IMPSymbol_%s.sti",
zLanguage, zLanguage ); return TRUE;
                }
        #endif
