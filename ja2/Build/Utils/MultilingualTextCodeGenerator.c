...
This code generator is used to conveniently compare the english master text file with another
foreign language such as German and verify that the appropriate language file is in perfect synch
with the English.  Verifying that all of the strings have the correct order of printf format
specifiers and the precise number.  If different, the errors are recorded via comments proceeding
the string in question in the new file.  For simplicity, the German language will be used in
examples throughout this documention.  The comments will be specially marked with "CONFLICT#xxx:
error message" which can be searched for.  The comment will report the format specifiers used in the
english version.
...
- Users don't use comments containing the { character later followed by the L" token.  The code
generator will mistaken that for a string.
...
BOOLEAN ProcessIfMultilingualCmdLineArgDetected(STR8 str) {
  ...
  if (strcmp(str, LCG_COMMANDLINEARGUMENT)) {  // string is different, so return
    return FALSE;
  }
  ...
  if (!SetFileManCurrentDirectory(
          Dir)) {  // We failed meaning the directory name is incorrect or non-existant
    AssertMsg(0,
              "Failed to set directory location while attempting to activate multilingual text "
              "code generator.");
    return FALSE;
  }
  ...
  if (!FileExists(LCG_ENGLISHMASTERFILE)) {
    AssertMsg(0,
              "Failed to find master english file while attempting to activate multilingual text "
              "code generator.");
    return FALSE;
  }
  if (!FileExists(LCG_FOREIGNMASTERFILE)) {
    AssertMsg(0,
              "Failed to find master foreign file while attempting to activate multilingual text "
              "code generator.");
    return FALSE;
  }
  ...
  if (uiEnglishStrings != uiForeignStrings) {
    AssertMsg(
        0,
        String("Mismatch during LCG preparation:  English DB strings: %d, Foreign DB strings: %d",
