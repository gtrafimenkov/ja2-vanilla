...
UINT32 ProcessAndEnterAHistoryRecord(UINT8 ubCode, UINT32 uiDate, UINT8 ubSecondCode,
                                     INT16 sSectorX, INT16 sSectorY, INT8 bSectorZ, UINT8 ubColor);
...
BOOLEAN IncrementCurrentPageHistoryDisplay(void) {
  ...
  //  if( ( FileGetSize( hFileHandle ) - 1 ) / ( NUM_RECORDS_PER_PAGE * ( sizeof( UINT8 ) + sizeof(
  //  UINT32 ) + 3*sizeof( UINT8 )+ sizeof(INT16) + sizeof( INT16 ) ) ) + 1 < ( UINT32 )(
...
void ProcessHistoryTransactionString(STR16 pString, HistoryUnitPtr pHistory) {
  ...
  switch (pHistory->ubCode) {
    ...
    case HISTORY_HIRED_MERC_FROM_AIM:
      swprintf(pString, pHistoryStrings[HISTORY_HIRED_MERC_FROM_AIM],
               gMercProfiles[pHistory->ubSecondCode].zName);
      break;

    case HISTORY_MERC_KILLED:
      if (pHistory->ubSecondCode != NO_PROFILE)
        swprintf(pString, pHistoryStrings[HISTORY_MERC_KILLED],
                 gMercProfiles[pHistory->ubSecondCode].zName);
    ...
    case HISTORY_HIRED_MERC_FROM_MERC:
      swprintf(pString, pHistoryStrings[HISTORY_HIRED_MERC_FROM_MERC],
               gMercProfiles[pHistory->ubSecondCode].zName);
      break;
    ...
    case (HISTORY_PURCHASED_INSURANCE):
      swprintf(pString, pHistoryStrings[HISTORY_PURCHASED_INSURANCE],
               gMercProfiles[pHistory->ubSecondCode].zNickname);
      break;
    case (HISTORY_CANCELLED_INSURANCE):
      swprintf(pString, pHistoryStrings[HISTORY_CANCELLED_INSURANCE],
               gMercProfiles[pHistory->ubSecondCode].zNickname);
      break;
    case (HISTORY_INSURANCE_CLAIM_PAYOUT):
      swprintf(pString, pHistoryStrings[HISTORY_INSURANCE_CLAIM_PAYOUT],
               gMercProfiles[pHistory->ubSecondCode].zNickname);
      break;

    case HISTORY_EXTENDED_CONTRACT_1_DAY:
      swprintf(pString, pHistoryStrings[HISTORY_EXTENDED_CONTRACT_1_DAY],
               gMercProfiles[pHistory->ubSecondCode].zNickname);
      break;

    case HISTORY_EXTENDED_CONTRACT_1_WEEK:
      swprintf(pString, pHistoryStrings[HISTORY_EXTENDED_CONTRACT_1_WEEK],
               gMercProfiles[pHistory->ubSecondCode].zNickname);
      break;

    case HISTORY_EXTENDED_CONTRACT_2_WEEK:
      swprintf(pString, pHistoryStrings[HISTORY_EXTENDED_CONTRACT_2_WEEK],
               gMercProfiles[pHistory->ubSecondCode].zNickname);
      break;

    case (HISTORY_MERC_FIRED):
      swprintf(pString, pHistoryStrings[HISTORY_MERC_FIRED],
               gMercProfiles[pHistory->ubSecondCode].zNickname);
      break;

    case (HISTORY_MERC_QUIT):
      swprintf(pString, pHistoryStrings[HISTORY_MERC_QUIT],
               gMercProfiles[pHistory->ubSecondCode].zNickname);
      break;
    ...
    case (HISTORY_TALKED_TO_MINER):
      swprintf(pString, pHistoryStrings[HISTORY_TALKED_TO_MINER],
               pTownNames[pHistory->ubSecondCode]);
      break;
    case (HISTORY_LIBERATED_TOWN):
      swprintf(pString, pHistoryStrings[HISTORY_LIBERATED_TOWN],
               pTownNames[pHistory->ubSecondCode]);
      break;
    ...
    case HISTORY_MERC_MARRIED_OFF:
      swprintf(pString, pHistoryStrings[HISTORY_MERC_MARRIED_OFF],
               gMercProfiles[pHistory->ubSecondCode].zNickname);
      break;
    case HISTORY_MERC_CONTRACT_EXPIRED:
      swprintf(pString, pHistoryStrings[HISTORY_MERC_CONTRACT_EXPIRED],
               gMercProfiles[pHistory->ubSecondCode].zName);
      break;
    case HISTORY_RPC_JOINED_TEAM:
      swprintf(pString, pHistoryStrings[HISTORY_RPC_JOINED_TEAM],
               gMercProfiles[pHistory->ubSecondCode].zName);
      break;
...
BOOLEAN LoadInHistoryRecords(UINT32 uiPage) {
  ...
  // is the file long enough?
  if ((FileGetSize(hFileHandle) - 1) / (NUM_RECORDS_PER_PAGE * SIZE_OF_HISTORY_FILE_RECORD) + 1 <
      uiPage) {
    // nope
    FileClose(hFileHandle);
    return (FALSE);
  ...
  FileSeek(hFileHandle, (uiPage - 1) * NUM_RECORDS_PER_PAGE * (SIZE_OF_HISTORY_FILE_RECORD),
