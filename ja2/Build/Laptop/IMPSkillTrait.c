...
enum {
  ...
  IMP__VOICE,
  IMP__FINISH,
};  // появилось в UB для удобства, в оригинале просто числа
...
};  // нужно для обработки выбранных навыков
...
void EnterIMPSkillTrait(void) {
  ...
  giIMPSkillTraitFinsihButton = CreateIconAndTextButton(
      giIMPSkillTraitFinsihButtonImage, pImpButtonText[11], FONT12ARIAL, FONT_WHITE, DEFAULT_SHADOW,
      FONT_WHITE, DEFAULT_SHADOW, TEXT_CJUSTIFIED, giOffsW + LAPTOP_SCREEN_UL_X + (350),
      giOffsH + LAPTOP_SCREEN_WEB_UL_Y + (340), BUTTON_TOGGLE, MSYS_PRIORITY_HIGH,
...
void AddImpSkillTraitButtons() {
  ...
  for (iCnt = 0; iCnt < IMP_SKILL_TRAITS__NUMBER_SKILLS; iCnt++) {
    ...
    if (iCnt == 0)
      giIMPSkillTraitAnswerButtonImage[iCnt] =
          LoadButtonImage("LAPTOP\\button_6.sti", -1, 0, -1, 1, -1);
    else
      giIMPSkillTraitAnswerButtonImage[iCnt] =
          UseLoadedButtonImage(giIMPSkillTraitAnswerButtonImage[0], -1, 0, -1, 1, -1);

    giIMPSkillTraitAnswerButton[iCnt] = QuickCreateButton(
        giIMPSkillTraitAnswerButtonImage[iCnt], giOffsW + usPosX, giOffsH + usPosY, BUTTON_TOGGLE,
        MSYS_PRIORITY_HIGHEST - 3, MSYS_NO_CALLBACK, (GUI_CALLBACK)BtnIMPSkillTraitAnswerCallback);
...
void HandleIMPSkillTraitAnswers(UINT32 uiSkillPressed, BOOLEAN fResetAllButtons) {
  ...
  /*
          //if there are already 2 skills selected, and we are trying to PRESS a new button ( skill
  isnt currently set ) if( CountNumSkillStraitsSelected() == 2 && !gfSkillTraitQuestions[
  uiSkillPressed ] && uiSkillPressed != IMP_SKILL_TRAITS__NONE )
          {
                  //if we have to, unselect an existing trait

                  //play the diosabld sound
  //		PlayButtonSound( giIMPSkillTraitAnswerButton[ uiSkillPressed ],
...
void IMPSkillTraitDisplaySkills() {
  ...
  DrawTextToScreen(gzIMPSkillTraitsText[IMP_SKILL_TRAIT__TITLE_TEXT],
                   giOffsW + IMP_SKILL_TRAIT__TITLE_X, giOffsH + IMP_SKILL_TRAIT__TITLE_Y,
                   IMP_SKILL_TRAIT__TITLE_WIDTH, IMP_SKILL_TRAIT__TITLE_FONT,
                   IMP_SKILL_TRAIT__COLOR, FONT_MCOLOR_BLACK, FALSE, CENTER_JUSTIFIED);
  ...
  for (uiCnt = 0; uiCnt < IMP_SKILL_TRAITS__NUMBER_SKILLS; uiCnt++) {
    ...
    if (gfSkillTraitQuestions[uiCnt]) {
      ...
      BltVideoObject(FRAME_BUFFER, hImageHandle, 1, giOffsW + usBoxPosX, giOffsH + usBoxPosY,
                     VO_BLT_SRCTRANSPARENCY, NULL);
    } else {
      // Display the grey background box
      BltVideoObject(FRAME_BUFFER, hImageHandle, 0, giOffsW + usBoxPosX, giOffsH + usBoxPosY,
                     VO_BLT_SRCTRANSPARENCY, NULL);
    }

    // draw the text to the screenx
    DrawTextToScreen(gzIMPSkillTraitsText[uiCnt], giOffsW + usPosX, giOffsH + usPosY, 0,
                     IMP_SKILL_TRAIT__FONT, IMP_SKILL_TRAIT__COLOR, FONT_MCOLOR_BLACK, FALSE,
                     LEFT_JUSTIFIED);
