...
BOOLEAN LightAddTile(UINT32 uiLightType, INT16 iSrcX, INT16 iSrcY, INT16 iX, INT16 iY,
                     UINT8 ubShade, UINT32 uiFlags, BOOLEAN fOnlyWalls);
BOOLEAN LightSubtractTile(UINT32 uiLightType, INT16 iSrcX, INT16 iSrcY, INT16 iX, INT16 iY,
                          UINT8 ubShade, UINT32 uiFlags, BOOLEAN fOnlyWalls);
...
BOOLEAN LightInsertNode(INT32 iLight, UINT16 usLightIns, INT16 iHotSpotX, INT16 iHotSpotY, INT16 iX,
                        INT16 iY, UINT8 ubIntensity, UINT16 uiFlags);
...
BOOLEAN LightCastRay(INT32 iLight, INT16 iStartX, INT16 iStartY, INT16 iEndPointX, INT16 iEndPointY,
                     UINT8 ubStartIntens, UINT8 ubEndIntens);
BOOLEAN LightGenerateElliptical(INT32 iLight, UINT8 iIntensity, INT16 iA, INT16 iB);
BOOLEAN LightGenerateBeam(INT32 iLight, UINT8 iIntensity, INT16 iLength, INT16 iRadius,
                          INT16 iDirection);
...
UINT16 LightAddRayNode(INT32 iLight, INT16 iX, INT16 iY, UINT8 ubLight, UINT16 usFlags) {
  ...
  if (pLightRayList[iLight] == NULL) {
    if ((pLightRayList[iLight] = (UINT16 *)MemAlloc(sizeof(UINT16))) == NULL) return (65535);
    ...
    *pLightRayList[iLight] = (LightAddTemplateNode(iLight, iX, iY, ubLight) | usFlags);
    ...
    pLightRayList[iLight] =
        (UINT16 *)MemRealloc(pLightRayList[iLight], (usNumNodes + 1) * sizeof(UINT16));
...
***************************************************************************************/
UINT16 LightInsertRayNode(INT32 iLight, UINT16 usIndex, INT16 iX, INT16 iY, UINT8 ubLight,
                          UINT16 usFlags) {
  UINT16 usNumNodes;
...
***************************************************************************************/
void LightSubtractTileNode(LEVELNODE *pNode, UINT32 uiLightType, UINT8 ubShadeSubtract,
                           BOOLEAN fFake) {
  INT16 sSum;
      ...
      if (pStruct->usIndex < NUMBEROFTILES) {
        if ((gTileDatabase[pStruct->usIndex].fType != FIRSTCLIFFHANG) ||
          ...
          if ((uiFlags & LIGHT_IGNORE_WALLS) || gfCaves)
            ...
            if (fLitWall && gfCaves) {
              LightAddTileNode(pStruct, uiLightType, (UINT8)__min(ubShadeAdd, (SHADE_MAX + 5)),
                               FALSE);
            } else if (fLitWall) {
              LightAddTileNode(pStruct, uiLightType, ubShadeAdd, FALSE);
            } else if (!fOnlyWalls) {
      ...
      if (pStruct->usIndex < NUMBEROFTILES) {
        if ((gTileDatabase[pStruct->usIndex].fType != FIRSTCLIFFHANG) ||
          ...
          if ((uiFlags & LIGHT_IGNORE_WALLS) || gfCaves)
            ...
            // ATE: Limit shade for walls if in caves
            if (fLitWall && gfCaves) {
              LightSubtractTileNode(pStruct, uiLightType, (UINT8)__max((ubShadeSubtract - 5), 0),
                                    FALSE);
            } else if (fLitWall) {
              LightSubtractTileNode(pStruct, uiLightType, ubShadeSubtract, FALSE);
            } else if (!fOnlyWalls) {
...
***************************************************************************************/
UINT16 LightGetLastNode(INT32 iLight) { return (usRaySize[iLight]); }
  ...
  /*
          if (!(LightSprites[uiSprite].uiFlags & MERC_LIGHT))
          {
                  ...
                  if (pMapElement->pLand != NULL)
                  {
                          ...
                          sSum=pMapElement->pLand->ubNaturalShadeLevel - __max(
...
BOOLEAN LightHideTrees(INT16 iX, INT16 iY) {
  ...
  // Kris:  added map boundary checking!!!
  for (iCountY = (INT16)__max(iY - LIGHT_TREE_REVEAL, 0);
       iCountY < (INT16)__min(iY + LIGHT_TREE_REVEAL, WORLD_ROWS - 1); iCountY++)
    for (iCountX = (INT16)__max(iX - LIGHT_TREE_REVEAL, 0);
         iCountX < (INT16)__min(iX + LIGHT_TREE_REVEAL, WORLD_COLS - 1); iCountX++) {
      uiTile = MAPROWCOLTOPOS(iCountY, iCountX);
      pNode = gpWorldLevelData[uiTile].pStructHead;
      while (pNode != NULL) {
        GetTileFlags(pNode->usIndex, &fTileFlags);
...
BOOLEAN LightSetColors(SGPPaletteEntry *pPal, UINT8 ubNumColors) {
  ...
  if (ubNumColors == 2) {
    sRed = __min(
        ...
        255);
    sGreen = __min((((INT16)pPal[0].peGreen) * LVL1_L1_PER / 100 +
    ...
    sRed = __min(
        ...
        255);
    sGreen = __min((((INT16)pPal[0].peGreen) * LVL2_L1_PER / 100 +
...
UINT16 CreateTilePaletteTables(HVOBJECT pObj, UINT32 uiTileIndex, BOOLEAN fForce) {
  ...
  if (!fLoaded) {  // This is expensive as hell to call!
    for (uiCount = 0; uiCount < 256; uiCount++) {
      ...
      LightPal[uiCount].peBlue = (UINT8)(__min(
          (UINT16)pObj->pPaletteEntry[uiCount].peBlue + (UINT16)gpLightColors[0].peBlue, 255));
    }
  ...
  if (gubNumLightColors == 2) {
    ...
    for (uiCount = 0; uiCount < 256; uiCount++) {
      ...
      LightPal[uiCount].peBlue = (UINT8)(__min(
          (UINT16)pObj->pPaletteEntry[uiCount].peBlue + (UINT16)gpLightColors[1].peBlue, 255));
    }
    ...
    for (uiCount = 0; uiCount < 256; uiCount++) {
      ...
      LightPal[uiCount].peBlue = (UINT8)(__min(
          (UINT16)pObj->pPaletteEntry[uiCount].peBlue + (UINT16)gpLightColors[2].peBlue, 255));
    }
...
UINT16 CreateSoldierPaletteTables(SOLDIERTYPE *pSoldier, UINT32 uiType) {
  ...
  for (uiCount = 0; uiCount < 256; uiCount++) {
    ...
    LightPal[uiCount].peBlue = (UINT8)(__min(
        (UINT16)pSoldier->p8BPPPalette[uiCount].peBlue + (UINT16)gpLightColors[0].peBlue, 255));
  }
  // build the shade tables
  ...
  if (gubNumLightColors == 2) {
    ...
    for (uiCount = 0; uiCount < 256; uiCount++) {
      LightPal[uiCount].peRed = (UINT8)(__min(
          (UINT16)pSoldier->p8BPPPalette[uiCount].peRed + (UINT16)gpLightColors[1].peRed, 255));
      LightPal[uiCount].peGreen = (UINT8)(__min(
          (UINT16)pSoldier->p8BPPPalette[uiCount].peGreen + (UINT16)gpLightColors[1].peGreen, 255));
      LightPal[uiCount].peBlue = (UINT8)(__min(
          (UINT16)pSoldier->p8BPPPalette[uiCount].peBlue + (UINT16)gpLightColors[1].peBlue, 255));
    }
    ...
    for (uiCount = 0; uiCount < 256; uiCount++) {
      LightPal[uiCount].peRed = (UINT8)(__min(
          (UINT16)pSoldier->p8BPPPalette[uiCount].peRed + (UINT16)gpLightColors[2].peRed, 255));
      LightPal[uiCount].peGreen = (UINT8)(__min(
          (UINT16)pSoldier->p8BPPPalette[uiCount].peGreen + (UINT16)gpLightColors[2].peGreen, 255));
      LightPal[uiCount].peBlue = (UINT8)(__min(
          (UINT16)pSoldier->p8BPPPalette[uiCount].peBlue + (UINT16)gpLightColors[2].peBlue, 255));
    }
