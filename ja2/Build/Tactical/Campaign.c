...
void ProfileStatChange(MERCPROFILESTRUCT *pProfile, UINT8 ubStat, UINT16 usNumChances,
  ...
  if (pProfile->bLife < OKLIFE) return;
...
void ProcessStatChange(MERCPROFILESTRUCT *pProfile, UINT8 ubStat, UINT16 usNumChances,
  ...
  for (uiCnt = 0; uiCnt < usNumChances; uiCnt++) {
    ...
    {
      // if this is improving from a failure, and a successful roll would give us enough to go up a
      // point
      if ((ubReason == FROM_FAILURE) && ((*psStatGainPtr + 1) >= usSubpointsPerPoint)) {
        // can't improve any more from this statchange, because Ian don't want failures causin
        // increases!
        break;
      }
      ...
      if (usChance > 0 && fAffectedByWisdom) {
        usChance +=
            (usChance *
             (pProfile->bWisdom +
              (pProfile->sWisdomGain / SubpointsPerPoint(WISDOMAMT, pProfile->bExpLevel)) - 50)) /
            100;
      }
      ...
      if (PreRandom(100) < usChance) {
        ...
        if ((ubStat != EXPERAMT) && (ubReason != FROM_TRAINING)) {
          ...
          if (uiEffLevel < MAXEXPLEVEL) {
            // if this is NOT improving from a failure, OR it would NOT give us enough to go up a
            // level
  ...
  if (sSubPointChange != 0) {
    // debug message
    ScreenMsg(MSG_FONT_RED, MSG_DEBUG, L"%s's %s changed by %d", pProfile->zNickname,
...
void ChangeStat(MERCPROFILESTRUCT *pProfile, SOLDIERTYPE *pSoldier, UINT8 ubStat,
  ...
  if (sPtsChanged != 0) {
    ...
    *pbStatPtr += sPtsChanged;
    ...
    // if this merc is currently on the player's team (DON'T count increases earned outside the
    // player's employ)
    if (pSoldier != NULL) {
    ...
    *psStatGainPtr = (*psStatGainPtr) % usSubpointsPerPoint;
    ...
    if (pSoldier != NULL) {
      ...
      *pbSoldierStatPtr = *pbStatPtr;
      ...
      // if ( (ubStat != EXPERAMT) && (ubStat != HEALTHAMT) && ( Random( 100 ) < 25 ) )
      {
        ...
        TacticalCharacterDialogueWithSpecialEventEx(pSoldier, 0,
      ...
      // remember what time it changed at, it's displayed in a different color for a while
      // afterwards
    ...
    if ((ubStat == EXPERAMT) && fChangeTypeIncrease) {
      ...
      if (pSoldier != NULL) {
        switch (pSoldier->ubWhatKindOfMercAmI) {
          ...
          case MERC_TYPE__MERC:
            ...
            // offset for the 2 profiles of Larry (we only have one email for Larry..but 2 profile
            // entries
      ...
      {
        // only AIM and M.E.R.C.s update stats when not on player's team, and both of them DO change
        // salary
        fChangeSalary = TRUE;
      }
...
void ProcessUpdateStats(MERCPROFILESTRUCT *pProfile, SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier != NULL) {
    // ATE: if in the midst of an attack, if in the field, delay all stat changes until the check
  ...
  for (ubStat = FIRST_CHANGEABLE_STAT; ubStat <= LAST_CHANGEABLE_STAT; ubStat++) {
    ...
    // Calc how many full points worth of stat changes we have accumulated in this stat (positive OR
    // negative!) NOTE: for simplicity, this hopes nobody will go up more than one level at once,
    // which would change the subpoints/pt
...
void HandleUnhiredMercImprovement(MERCPROFILESTRUCT *pProfile) {
  ...
  if (pProfile->bMercStatus == MERC_WORKING_ELSEWHERE) {
    ...
    // so about 10 working days to hit lvl 2.  This seems high, but mercs don't actually "work" that
    // often, and it's twice as long to hit level 3.  If we go lower, attribs & skills will barely
    // move.
    ...
    do {
      // pick ONE stat at random to focus on (it may be beyond training cap, but so what, too hard
      // to weed those out)
...
void HandleUnhiredMercDeaths(INT32 iProfileID) {
  ...
  if (!(pProfile->ubMiscFlags3 & PROFILE_MISC_FLAG3_PLAYER_HAD_CHANCE_TO_HIRE)) {
    // then we're not allowed to kill him (to avoid really pissing off player by killing his very
    // favorite merc)
    return;
  }
...
void TestDumpStatChanges(void) {
  ...
  fprintf(FDump,
          "---HEALTH-- --AGILITY-- -DEXTERITY- ---WISDOM-- --MEDICAL-- --EXPLOSIV- --MECHANIC- "
          "--MARKSMAN- -EXP.LEVEL- --STRENGTH- -LEADERSHIP");
  fprintf(FDump, "\n");
    ...
    if (fMercUsed) {
      ...
      for (ubStat = FIRST_CHANGEABLE_STAT; ubStat <= LAST_CHANGEABLE_STAT; ubStat++) {
        if (pProfile->usStatChangeChances[ubStat] > 0) {
          ...
          fprintf(FDump, " %5d/%-5d", pProfile->usStatChangeSuccesses[ubStat],
                  pProfile->usStatChangeChances[ubStat]);
        } else {
          //
          fprintf(FDump, "            ");
        }
...
void BuildStatChangeString(STR16 wString, STR16 wName, BOOLEAN fIncrease, INT16 sPtsChanged,
  ...
  swprintf(wString, L"%s %s %d %s %s", wName, sPreStatBuildString[fIncrease ? 1 : 0],
