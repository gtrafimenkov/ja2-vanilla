  ...
  */
  ...
  for (iCount = 0; iCount < giNumRottingCorpse; iCount++) {
    if (gRottingCorpse[iCount].fActivated == TRUE) {
      ...
      FileWrite(hFile, &gRottingCorpse[iCount].def, sizeof(ROTTING_CORPSE_DEFINITION),
                &uiNumBytesWritten);
...
BOOLEAN GetNumberOfActiveWorldItemsFromTempFile(INT16 sMapX, INT16 sMapY, INT8 bMapZ,
  ...
  // If there is a file, load in the Items array
  if (bMapZ == 0) {
    if (SectorInfo[SECTOR(sMapX, sMapY)].uiFlags & SF_ITEM_TEMP_FILE_EXISTS) fFileLoaded = TRUE;
  } else {
    while (TempNode) {
...
UINT32 GetNumberOfVisibleWorldItemsFromSectorStructureForSector(INT16 sMapX, INT16 sMapY,
  ...
  if (bMapZ == 0) {
    uiNumberOfItems =
        SectorInfo[SECTOR(sMapX, sMapY)].uiNumberOfWorldItemsInTempFileThatCanBeSeenByPlayer;
  } else {
