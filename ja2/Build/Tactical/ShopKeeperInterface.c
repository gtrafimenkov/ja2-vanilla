...
#define MAX_SUBOBJECTS_PER_OBJECT \
  max(MAX_OBJECTS_PER_SLOT,       \
      (2 + MAX_ATTACHMENTS))  // (2nd part is main item, ammo/payload, and 4 attachments)
...
void StoreObjectsInNextFreeDealerInvSlot(UINT16 usItemIndex, SPECIAL_ITEM_INFO *pSpclItemInfo,
                                         INT16 sSpecialItemElement, UINT8 ubHowMany, UINT8 ubOwner);
void AddItemsToTempDealerInventory(UINT16 usItemIndex, SPECIAL_ITEM_INFO *pSpclItemInfo,
                                   INT16 sSpecialItemElement, UINT8 ubHowMany, UINT8 ubOwner);
...
UINT32 DisplayInvSlot(UINT8 ubSlotNum, UINT16 usItemIndex, UINT16 usPosX, UINT16 usPosY,
                      OBJECTTYPE *ItemObject, BOOLEAN fHatchedOut, UINT8 ubItemArea);
...
UINT32 CalcShopKeeperItemPrice(BOOLEAN fDealerSelling, BOOLEAN fUnitPriceOnly, UINT16 usItemID,
                               FLOAT dModifier, OBJECTTYPE *pItemObject);
...
void CountSubObjectsInObject(OBJECTTYPE *pComplexObject, UINT8 *pubTotalSubObjects,
                             UINT8 *pubRepairableSubObjects, UINT8 *pubNonRepairableSubObjects);
BOOLEAN AddObjectForEvaluation(OBJECTTYPE *pObject, UINT8 ubOwnerProfileId, INT8 bOwnerSlotId,
                               BOOLEAN fFirstOne);
...
UINT32 ShopKeeperScreenHandle() {
  ...
  if (gfDisplayNoRoomMsg) {
    ...
    // ARM: message is delayed because we need the mouse restriction to be in place BEFORE it comes
    // up so it gets lifted/restored
    DoSkiMessageBox(MSG_BOX_BASIC_STYLE, SKI_Text[SKI_TEXT_NO_MORE_ROOM_IN_PLAYER_OFFER_AREA],
                    SHOPKEEPER_SCREEN, MSG_BOX_FLAG_OK, NULL);
...
BOOLEAN EnterShopKeeperInterface() {
  ...
  // Check to make sure the inventory is null ( should always be null if we are just coming in to
  // the SKI )
  ...
  */
  ...
  // faces
  // This is to support showing of repair item owner's faces even when they're not in the sector, as
  // long as they still work for player
  gubNumberMercsInArray = 0;
  for (ubCnt = gTacticalStatus.Team[OUR_TEAM].bFirstID;
  ...
  guiSKI_InvPageUpButtonImage =
      LoadButtonImage("INTERFACE\\TradeScrollArrows.sti", -1, 0, -1, 1, -1);
  guiSKI_InvPageDownButtonImage =
      UseLoadedButtonImage(guiSKI_InvPageUpButtonImage, -1, 2, -1, 3, -1);

  // Page up button for the merchant inventory
  guiSKI_InvPageUpButton = QuickCreateButton(
      guiSKI_InvPageUpButtonImage, SKI_PAGE_UP_ARROWS_X, SKI_PAGE_UP_ARROWS_Y, BUTTON_NEWTOGGLE,
      MSYS_PRIORITY_HIGHEST, DEFAULT_MOVE_CALLBACK, BtnSKI_InvPageUpButtonCallback);
  SpecifyDisabledButtonStyle(guiSKI_InvPageUpButton, DISABLED_STYLE_HATCHED);

  // Page down button for the merchant inventory
  guiSKI_InvPageDownButton =
      QuickCreateButton(guiSKI_InvPageDownButtonImage, SKI_PAGE_DOWN_ARROWS_X,
                        SKI_PAGE_DOWN_ARROWS_Y, BUTTON_NEWTOGGLE, MSYS_PRIORITY_HIGHEST,
  ...
  //	guiSKI_TransactionButtonImage = UseLoadedButtonImage( guiSKI_EvaluateButtonImage,
  //-1,0,-1,1,-1 );
  ...
  //Evaluate:
          //Evaluate button
          guiSKI_EvaluateButton = CreateIconAndTextButton( guiSKI_EvaluateButtonImage,
  SKI_Text[SKI_TEXT_EVALUATE], SKI_BUTTON_FONT, SKI_BUTTON_COLOR, DEFAULT_SHADOW, SKI_BUTTON_COLOR,
  DEFAULT_SHADOW, TEXT_CJUSTIFIED, SKI_EVALUATE_BUTTON_X, SKI_EVALUATE_BUTTON_Y, BUTTON_TOGGLE,
  MSYS_PRIORITY_HIGH, DEFAULT_MOVE_CALLBACK, BtnSKI_EvaluateButtonCallback);
          SpecifyDisabledButtonStyle( guiSKI_EvaluateButton, DISABLED_STYLE_HATCHED );
  */

  // Transaction button
  guiSKI_TransactionButton = CreateIconAndTextButton(
      guiSKI_TransactionButtonImage, SKI_Text[SKI_TEXT_TRANSACTION], SKI_BUTTON_FONT,
  ...
  if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS)
    SetButtonFastHelpText(guiSKI_TransactionButton,
                          SkiMessageBoxText[SKI_REPAIR_TRANSACTION_BUTTON_HELP_TEXT]);
  else
    SetButtonFastHelpText(guiSKI_TransactionButton,
                          SkiMessageBoxText[SKI_TRANSACTION_BUTTON_HELP_TEXT]);

  // Done button
  guiSKI_DoneButton = CreateIconAndTextButton(
      guiSKI_DoneButtonImage, SKI_Text[SKI_TEXT_DONE], SKI_BUTTON_FONT, SKI_BUTTON_COLOR,
      DEFAULT_SHADOW, SKI_BUTTON_COLOR, DEFAULT_SHADOW, TEXT_CJUSTIFIED, SKI_DONE_BUTTON_X,
      SKI_DONE_BUTTON_Y, BUTTON_TOGGLE, MSYS_PRIORITY_HIGH + 10, DEFAULT_MOVE_CALLBACK,
  ...
  //ATM:
          ...
          MSYS_DefineRegion( &gShopKeeperCoverTacticalButtonMouseRegion, SKI_ATM_PANEL_X,
  SKI_ATM_PANEL_Y, (UINT16)(SKI_ATM_PANEL_X+SKI_ATM_PANEL_WIDTH),
  (UINT16)(SKI_ATM_PANEL_Y+SKI_ATM_PANEL_HEIGHT), MSYS_PRIORITY_HIGH-1, CURSOR_NORMAL,
  MSYS_NO_CALLBACK, MSYS_NO_CALLBACK); MSYS_AddRegion( &gSKI_EntireScreenMouseRegions );
  */
  ...
  if (gItemToAdd.fActive) {
    ...
    // if this is NOT a repair dealer or he is is but there is enough space in the player's offer
    // area
    // (you can't be out of space if it isn't a repairman, only they can fill it up with repaired
    // items!)
    if ((ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer != ARMS_DEALER_REPAIRS) ||
...
#ifdef JA2TESTVERSION
  if (gfTestDisplayDealerCash) DisplayAllDealersCash();
#endif
...
BOOLEAN RenderShopKeeperInterface() {
  ...
  DrawTextToScreen(SKI_Text[SKI_TEXT_MERCHADISE_IN_STOCK], SKI_MAIN_TITLE_X, SKI_MAIN_TITLE_Y,
                   SKI_MAIN_TITLE_WIDTH, SKI_TITLE_FONT, SKI_TITLE_COLOR, FONT_MCOLOR_BLACK, FALSE,
                   CENTER_JUSTIFIED);
  ...
  DisplayWrappedString(SKI_PLAYERS_CURRENT_BALANCE_X, SKI_PLAYERS_CURRENT_BALANCE_Y,
                       ...
                       SkiMessageBoxText[SKI_PLAYERS_CURRENT_BALANCE], FONT_MCOLOR_BLACK, FALSE,
                       CENTER_JUSTIFIED);
  ...
  if (gfRenderScreenOnNextLoop) {
    //	BlitBufferToBuffer( guiRENDERBUFFER, guiCornerWhereTacticalIsStillSeenImage,
...
void RestoreTacticalBackGround() {
  ...
  //	BlitBufferToBuffer( guiCornerWhereTacticalIsStillSeenImage, guiRENDERBUFFER,
  // SKI_TACTICAL_BACKGROUND_START_X, SKI_TACTICAL_BACKGROUND_START_Y,
  // SKI_TACTICAL_BACKGROUND_START_WIDTH, SKI_TACTICAL_BACKGROUND_START_HEIGHT );
...
void DisplayAllDealersCash() {
  ...
  for (bArmsDealer = 0; bArmsDealer < NUM_ARMS_DEALERS; bArmsDealer++) {
    ...
    DrawTextToScreen(gMercProfiles[ArmsDealerInfo[bArmsDealer].ubShopKeeperID].zNickname, 540,
                     usPosY, 0, FONT10ARIAL, SKI_TITLE_COLOR, FONT_MCOLOR_BLACK, TRUE,
                     LEFT_JUSTIFIED);
    ...
    ubForeColor =
        (UINT8)((bArmsDealer == gbSelectedArmsDealerID) ? SKI_BUTTON_COLOR : SKI_TITLE_COLOR);
    DrawTextToScreen(zTemp, 590, usPosY, 0, FONT10ARIAL, ubForeColor, FONT_MCOLOR_BLACK, TRUE,
                     LEFT_JUSTIFIED);
    usPosY += 17;
  }
}
...
void BtnSKI_DoneButtonCallback(GUI_BUTTON *btn, INT32 reason) {
  ...
  InvalidateRegion(btn->Area.RegionTopLeftX, btn->Area.RegionTopLeftY, btn->Area.RegionBottomRightX,
...
void CreateSkiInventorySlotMouseRegions() {
  ...
  for (y = 0; y < SKI_NUM_ARMS_DEALERS_INV_ROWS; y++) {
    ...
    for (x = 0; x < SKI_NUM_ARMS_DEALERS_INV_COLS; x++) {
      MSYS_DefineRegion(&gDealersInventoryMouseRegions[ubCnt], usPosX, usPosY,
      ...
      if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
        // Small Faces
        MSYS_DefineRegion(&gRepairmanInventorySmallFaceMouseRegions[ubCnt],
                          (UINT16)(usPosX + SKI_SMALL_FACE_OFFSET_X), (UINT16)(usPosY),
                          (INT16)(usPosX + SKI_SMALL_FACE_OFFSET_X + SKI_SMALL_FACE_WIDTH),
                          (INT16)(usPosY + SKI_SMALL_FACE_HEIGHT), MSYS_PRIORITY_HIGH + 1,
        ...
        MSYS_SetRegionUserData(&gRepairmanInventorySmallFaceMouseRegions[ubCnt], 1,
  ...
  for (y = 0; y < SKI_NUM_TRADING_INV_ROWS; y++) {
    ...
    for (x = 0; x < SKI_NUM_TRADING_INV_COLS; x++) {
      MSYS_DefineRegion(&gDealersOfferSlotsMouseRegions[ubCnt], usPosX, usPosY,
                        (INT16)(usPosX + SKI_INV_SLOT_WIDTH), (INT16)(usPosY + SKI_INV_SLOT_HEIGHT),
                        MSYS_PRIORITY_HIGH, CURSOR_NORMAL,
      ...
      if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
        // Small Faces
        MSYS_DefineRegion(&gDealersOfferSlotsSmallFaceMouseRegions[ubCnt],
                          (UINT16)(usPosX + SKI_SMALL_FACE_OFFSET_X), (UINT16)(usPosY),
                          (INT16)(usPosX + SKI_SMALL_FACE_OFFSET_X + SKI_SMALL_FACE_WIDTH),
                          (INT16)(usPosY + SKI_SMALL_FACE_HEIGHT), MSYS_PRIORITY_HIGH + 1,
                          CURSOR_NORMAL, SelectDealersOfferSlotsMovementRegionCallBack,
                          SelectDealersOfferSlotsRegionCallBack);
        ...
        MSYS_SetRegionUserData(&gDealersOfferSlotsSmallFaceMouseRegions[ubCnt], 1,
  ...
  for (y = 0; y < SKI_NUM_TRADING_INV_ROWS; y++) {
    ...
    for (x = 0; x < SKI_NUM_TRADING_INV_COLS; x++) {
      // Trading Slots
      MSYS_DefineRegion(&gPlayersOfferSlotsMouseRegions[ubCnt], usPosX, usPosY,
                        (INT16)(usPosX + SKI_INV_SLOT_WIDTH), (INT16)(usPosY + SKI_INV_SLOT_HEIGHT),
                        MSYS_PRIORITY_HIGH, CURSOR_NORMAL,
      ...
      /*
                              //if the dealer repairs
                              if( ArmsDealerInfo[ gbSelectedArmsDealerID ].ubTypeOfArmsDealer ==
         ARMS_DEALER_REPAIRS )
                              {
                                      MSYS_SetRegionUserData(
         &gPlayersOfferSlotsSmallFaceMouseRegions[ ubCnt ], 1, PLAYERS_OFFER_AREA );
                              }
      */
      // Small Faces
      MSYS_DefineRegion(&gPlayersOfferSlotsSmallFaceMouseRegions[ubCnt],
                        (UINT16)(usPosX + SKI_SMALL_FACE_OFFSET_X), (UINT16)(usPosY),
                        (INT16)(usPosX + SKI_SMALL_FACE_OFFSET_X + SKI_SMALL_FACE_WIDTH),
                        (INT16)(usPosY + SKI_SMALL_FACE_HEIGHT), MSYS_PRIORITY_HIGH + 1,
                        CURSOR_NORMAL, SelectPlayersOfferSlotsMovementRegionCallBack,
                        SelectPlayersOfferSlotsRegionCallBack);
      ...
      MSYS_SetRegionUserData(&gPlayersOfferSlotsSmallFaceMouseRegions[ubCnt], 1,
                             PLAYERS_OFFER_AREA);
  ...
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
    ...
    if (gpTempDealersInventory[ubSelectedInvSlot].ItemObject.ubNumberOfObjects > 0) {
      ...
      if (!(gpTempDealersInventory[ubSelectedInvSlot].uiFlags & ARMS_INV_ITEM_SELECTED)) {
        // if the dealer repairs
        if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
          // ignore left clicks on items under repair.  Fully repaired items are moved out to
          // player's slots automatically
    ...
    /*
       ...
       )
                    {
                            ...
                            if( ArmsDealerOfferArea[ gpTempDealersInventory[ ubSelectedInvSlot
       ].bSlotIdInOtherLocation ].ItemObject.ubNumberOfObjects > 0 )
                            {
                                    //Increase the number in the dealer inventory
                                    gpTempDealersInventory[ ubSelectedInvSlot
       ].ItemObject.ubNumberOfObjects ++;

                                    //Decrease the number in the dealer offer area
                                    ArmsDealerOfferArea[ gpTempDealersInventory[ ubSelectedInvSlot
       ].bSlotIdInOtherLocation ].ItemObject.ubNumberOfObjects--;

                                    //if there is nothing left in the arms dealer offer area
                                    if( ArmsDealerOfferArea[ gpTempDealersInventory[
       ubSelectedInvSlot ].bSlotIdInOtherLocation ].ItemObject.ubNumberOfObjects == 0 )
                                    {
...
void SelectDealersInventoryMovementRegionCallBack(MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
    ...
    if (gpTempDealersInventory[ubSelectedInvSlot].fActive == FALSE) return;
...
void SelectDealersOfferSlotsMovementRegionCallBack(MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
    ...
    // if there is nothing in the slot, exit
    if (ArmsDealerOfferArea[ubSelectedInvSlot].fActive == FALSE) return;
...
void SelectPlayersOfferSlotsMovementRegionCallBack(MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
    ...
    // if there is nothing in the slot, exit
    if (PlayersOfferArea[ubSelectedInvSlot].fActive == FALSE) return;
  ...
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
    ...
    if (ArmsDealerOfferArea[ubSelectedInvSlot].fActive) {
      ...
      if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
        ...
        // bring up the item description box
        if (!InItemDescriptionBox()) {
          InitShopKeeperItemDescBox(&ArmsDealerOfferArea[ubSelectedInvSlot].ItemObject,
                                    ubSelectedInvSlot, ARMS_DEALER_OFFER_AREA);
        } else {
          DeleteItemDescriptionBox();
        }
      }
  ...
  } else if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP)  // MSYS_CALLBACK_REASON_LBUTTON_UP)
  {
    ...
    */
    ...
    if (ArmsDealerOfferArea[ubSelectedInvSlot].fActive) {
      ...
      if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
        ...
        if (gMoveingItem.sItemIndex == 0) {
          ...
          if (ArmsDealerOfferArea[ubSelectedInvSlot].ItemObject.ubNumberOfObjects >
              MAX_OBJECTS_PER_SLOT) {
            // then ignore the click - we can't do the swap, or anything very useful, because we
            // can't allow overloaded items into the player's cursor - if he put them into a merc's
            // inventory, the extra statuses are missing! At best, we could do some sort of message
            // here.
            return;
          }
      ...
      } else  // non-repairman
      {
        // if the shift key is being pressed, remove them all
        if (gfKeyState[SHIFT] ||
            (ArmsDealerOfferArea[ubSelectedInvSlot].ItemObject.ubNumberOfObjects == 1)) {
          RemoveItemFromArmsDealerOfferArea(
              ubSelectedInvSlot,
              TRUE);  // ArmsDealerOfferArea[ ubSelectedInvSlot ].bSlotIdInOtherLocation
        } else        // multiple items there, SHIFT isn't being pressed
        {
          ...
          gpTempDealersInventory[ArmsDealerOfferArea[ubSelectedInvSlot].bSlotIdInOtherLocation]
              .ItemObject.ubNumberOfObjects++;
        }
    ...
    } else  // empty slot
    {
      ...
      if (gMoveingItem.sItemIndex > 0) {
        // we'd better talking to a repairman, cursor is locked out of this area while full for
        // non-repairmen!
...
void SelectPlayersOfferSlotsRegionCallBack(MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
    ...
    {
      // Drop the item into the current slot
      //			AddObjectToSkiLocation( &gpSkiItemPointer, ubSelectedInvSlot,
      // PLAYERS_OFFER_AREA );
      ...
      if (PlayersOfferArea[ubSelectedInvSlot].fActive) {
        ...
        // if the slot is overloaded (holds more objects than we have room for valid statuses of)
        if (PlayersOfferArea[ubSelectedInvSlot].ItemObject.ubNumberOfObjects >
            MAX_OBJECTS_PER_SLOT) {
          // then ignore the click - we can't do the swap, or anything very useful, because we can't
          // allow overloaded items into the player's cursor - if he put them into a merc's
          // inventory, the extra statuses are missing! At best, we could do some sort of message
          // here.
          return;
        }
        ...
        if (Item[PlayersOfferArea[ubSelectedInvSlot].sItemIndex].usItemClass == IC_MONEY) {
          ...
          PlayersOfferArea[ubSelectedInvSlot].uiItemPrice =
              PlayersOfferArea[ubSelectedInvSlot].ItemObject.bMoneyStatus;
        }
      } else  // slot is empty
      {
  ...
  } else if (iReason & MSYS_CALLBACK_REASON_RBUTTON_UP)  // MSYS_CALLBACK_REASON_LBUTTON_UP)
  {
    ...
    if (PlayersOfferArea[ubSelectedInvSlot].fActive) {
      ...
      (UINT8)PlayersOfferArea[ ubSelectedInvSlot ].bSlotIdInOtherLocation, 214, 1 +
      INV_INTERFACE_START_Y, 0 );
                                      }
                                      else
                                      {
                                              DeleteItemDescriptionBox( );
...
#ifdef JA2BETAVERSION
    ScreenMsg(FONT_MCOLOR_WHITE, MSG_BETAVERSION, L"Failed to find Arms Dealer ID From Merc ID #%d",
              ubArmsDealer);
#endif
    gfSKIScreenExit = TRUE;
  }
...
void InitializeShopKeeper(BOOLEAN fResetPage) {
  ...
  if (gSelectArmsDealerInfo.uiNumDistinctInventoryItems == 0)
    ...
    else if (gSelectArmsDealerInfo.ubCurrentPage < 0 ||
...
void DisplayArmsDealerCurrentInventoryPage() {
  ...
  if (gpTempDealersInventory != NULL) {
    if (gubSkiDirtyLevel != SKI_DIRTY_LEVEL0) {
      ...
      DrawTextToScreen(SKI_Text[SKI_TEXT_PAGE], SKI_PAGE_X, (UINT16)(SKI_PAGE_Y + 3),
                       SKI_PAGE_WIDTH, SKI_LABEL_FONT, SKI_TITLE_COLOR, FONT_MCOLOR_BLACK, FALSE,
                       CENTER_JUSTIFIED);

      // Display the Current Page number
      uiFontHeight = GetFontHeight(SKI_LABEL_FONT);
      swprintf(zTemp, L"%d/%d", gSelectArmsDealerInfo.ubCurrentPage,
               gSelectArmsDealerInfo.ubNumberOfPages);
      DrawTextToScreen(zTemp, SKI_PAGE_X, (UINT16)(SKI_PAGE_Y + uiFontHeight + 6), SKI_PAGE_WIDTH,
                       SKI_LABEL_FONT, SKI_TITLE_COLOR, FONT_MCOLOR_BLACK, FALSE, CENTER_JUSTIFIED);
    }
    ...
    for (usCnt = gSelectArmsDealerInfo.ubFirstItemIndexOnPage;
      ...
      if (gpTempDealersInventory[usCnt].sItemIndex != 0) {
        ...
        DisplayInvSlot((UINT8)usCnt, gpTempDealersInventory[usCnt].sItemIndex, usPosX, usPosY,
                       &gpTempDealersInventory[usCnt].ItemObject, fDisplayHatchOnItem,
                       ARMS_DEALER_INVENTORY);
  ...
  if (gubSkiDirtyLevel == SKI_DIRTY_LEVEL2) {
    // This handles the remaining (empty) slots, resetting Fast Help text, and hatching out disabled
    // ones
...
UINT32 DisplayInvSlot(UINT8 ubSlotNum, UINT16 usItemIndex, UINT16 usPosX, UINT16 usPosY,
                      OBJECTTYPE *pItemObject, BOOLEAN fHatchedOut, UINT8 ubItemArea) {
  CHAR16 zTemp[64];
  HVOBJECT hVObject;
  HVOBJECT hPixHandle;
  INVTYPE *pItem;
  ...
  if (!(fHighlighted || gubSkiDirtyLevel != SKI_DIRTY_LEVEL0)) return (0);
  ...
  if (ubItemArea == PLAYERS_OFFER_AREA) {
    ...
    if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer != ARMS_DEALER_REPAIRS) {
      if (fHatchedOut && pItemObject->ubNumberOfObjects == 0) {
        ...
        uiItemCost = CalcShopKeeperItemPrice(DEALER_SELLING, TRUE, usItemIndex,
                                             ArmsDealerInfo[gbSelectedArmsDealerID].dSellModifier,
                                             pItemObject);
      }
    } else  // UNDER REPAIR
    {
  ...
  } else  // DEALER'S OFFER AREA
  {
    if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
      ...
      uiItemCost = CalcShopKeeperItemPrice(DEALER_SELLING, FALSE, usItemIndex,
                                           ArmsDealerInfo[gbSelectedArmsDealerID].dSellModifier,
                                           pItemObject);
  }
  ...
  // if the there is more then 1 or if the item is stackable and some of it has been bought and only
  // 1 remains
  ...
  if ((pItemObject->ubNumberOfObjects > 1) ||
      ((pItemObject->ubNumberOfObjects == 1) && DealerItemIsSafeToStack(usItemIndex) &&
       ...
       (gpTempDealersInventory[ubSlotNum].uiFlags & ARMS_INV_ITEM_SELECTED))) {
    swprintf(zTemp, L"x%d", pItemObject->ubNumberOfObjects);
    DrawTextToScreen(zTemp, (UINT16)(usPosX + SKI_ITEM_NUMBER_TEXT_OFFSET_X),
                     (UINT16)(usPosY + SKI_ITEM_NUMBER_TEXT_OFFSET_Y), SKI_ITEM_NUMBER_TEXT_WIDTH,
                     SKIT_NUMBER_FONT, SKI_ITEM_PRICE_COLOR, FONT_MCOLOR_BLACK, FALSE,
                     RIGHT_JUSTIFIED);
  }
  ...
  if (ItemHasAttachments(pItemObject)) {
    ...
    swprintf(zTemp, L"*");
    DrawTextToScreen(zTemp, (UINT16)(usPosX + SKI_ATTACHMENT_SYMBOL_X_OFFSET),
                     (UINT16)(usPosY + SKI_ATTACHMENT_SYMBOL_Y_OFFSET), 0, TINYFONT1, FONT_GREEN,
  ...
  if (pItemObject->bGunAmmoStatus < 0) {
    ...
    VarFindFontCenterCoordinates(usPosX, usPosY, SKI_INV_SLOT_WIDTH, SKI_INV_HEIGHT, TINYFONT1,
                                 &sCenX, &sCenY, zTemp);
    DrawTextToScreen(zTemp, sCenX, sCenY, SKI_INV_SLOT_WIDTH, TINYFONT1, FONT_RED,
                     FONT_MCOLOR_BLACK, FALSE, LEFT_JUSTIFIED);
...
BOOLEAN DetermineArmsDealersSellingInventory() {
  ...
  gpTempDealersInventory = (INVENTORY_IN_SLOT *)MemAlloc(
      sizeof(INVENTORY_IN_SLOT) * gSelectArmsDealerInfo.uiNumDistinctInventoryItems);
  if (gpTempDealersInventory == NULL) {
    Assert(0);
    return (FALSE);
  }
  memset(gpTempDealersInventory, 0,
         sizeof(INVENTORY_IN_SLOT) * gSelectArmsDealerInfo.uiNumDistinctInventoryItems);
...
void AddItemsToTempDealerInventory(UINT16 usItemIndex, SPECIAL_ITEM_INFO *pSpclItemInfo,
  ...
  } else  // more than one
  {
    ...
    if (DealerItemIsSafeToStack(usItemIndex)) {
      // then we can store them all together in the same box safely, even if there's more than
      // MAX_OBJECTS_PER_SLOT
      StoreObjectsInNextFreeDealerInvSlot(usItemIndex, pSpclItemInfo, sSpecialItemElement,
                                          ubHowMany, ubOwner);
    } else {
      // non-stacking items must be stored in one / box , because each may have unique fields
      // besides bStatus[] Example: guns all have ammo, ammo type, etc.  We need these uniquely
      // represented for pricing & manipulation
...
void StoreObjectsInNextFreeDealerInvSlot(UINT16 usItemIndex, SPECIAL_ITEM_INFO *pSpclItemInfo,
  ...
  // can't use the real #, because CreateItems() will blindly set the bStatus for however many we
  // tell it, beyond 8
...
BOOLEAN RepairIsDone(UINT16 usItemIndex, UINT8 ubElement) {
  ...
  for (ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++) {
    if (RepairItem.ItemObject.usAttachItem[ubCnt] != NONE) {
      /* ARM: Can now repair with removeable attachments still attached...
                              ...
                              if ( Item[ RepairItem.ItemObject.usAttachItem[ ubCnt ] ].fFlags &
         ITEM_INSEPARABLE )
      */
  ...
  if (/*( gArmsDealersInventory[ gbSelectedArmsDealerID ][ usItemIndex ].SpecialItem[ ubElement
         ].Info.ubImprintID == (NO_PROFILE + 1) ) && */
...
UINT32 CalcShopKeeperItemPrice(BOOLEAN fDealerSelling, BOOLEAN fUnitPriceOnly, UINT16 usItemID,
  ...
  switch (Item[usItemID].usItemClass) {
    ...
    default:
      ...
      if (pItemObject->ubNumberOfObjects <= MAX_OBJECTS_PER_SLOT) {
        ...
        ubItemsNotCounted = 0;
        // in this situation, uiUnitPrice will actually be the sum of the values of ALL the multiple
        // objects
      } else {
  ...
  // if he is buying this from a player, then we allow a value of 0, since that has a special
  // "worthless" quote #18
...
void DisplayArmsDealerOfferArea() {
  ...
  for (sCnt = 0; sCnt < SKI_NUM_TRADING_INV_SLOTS; sCnt++) {
    if (ArmsDealerOfferArea[sCnt].fActive) {
      ...
      // Display the inventory slot
      uiTotalCost += DisplayInvSlot((UINT8)sCnt, ArmsDealerOfferArea[sCnt].sItemIndex, usPosX,
                                    usPosY, &ArmsDealerOfferArea[sCnt].ItemObject,
  ...
  if (gubSkiDirtyLevel == SKI_DIRTY_LEVEL2) {
    ...
    InsertCommasForDollarFigure(zTemp);
    InsertDollarSignInToString(zTemp);
    DrawTextToScreen(zTemp, SKI_ARMS_DEALER_TOTAL_COST_X,
                     (UINT16)(SKI_ARMS_DEALER_TOTAL_COST_Y + 5), SKI_INV_SLOT_WIDTH, SKI_LABEL_FONT,
                     SKI_ITEM_PRICE_COLOR, FONT_MCOLOR_BLACK, FALSE, CENTER_JUSTIFIED);
  }
}
...
INT8 AddItemToArmsDealerOfferArea(INVENTORY_IN_SLOT *pInvSlot, INT8 bSlotIdInOtherLocation) {
  ...
  for (bCnt = 0; bCnt < SKI_NUM_TRADING_INV_SLOTS; bCnt++) {
    ...
    if (ArmsDealerOfferArea[bCnt].fActive == FALSE) {
      ...
      if (gfKeyState[SHIFT])
        ArmsDealerOfferArea[bCnt].ItemObject.ubNumberOfObjects =
      ...
      SetSkiRegionHelpText(&ArmsDealerOfferArea[bCnt], &gDealersOfferSlotsMouseRegions[bCnt],
                           ARMS_DEALER_OFFER_AREA);
      SetSkiFaceRegionHelpText(&ArmsDealerOfferArea[bCnt],
                               &gDealersOfferSlotsSmallFaceMouseRegions[bCnt],
                               ARMS_DEALER_OFFER_AREA);
...
BOOLEAN RemoveItemFromArmsDealerOfferArea(INT8 bSlotId, BOOLEAN fKeepItem) {
  ...
  //	for( usCnt=0; usCnt<SKI_NUM_TRADING_INV_SLOTS; usCnt++)
  {
    ...
    if (ArmsDealerOfferArea[bSlotId].fActive)  // bSlotIdInOtherLocation == bSlotIdInOtherLocation )
    {
      ...
      if (fKeepItem) {
        gpTempDealersInventory[ArmsDealerOfferArea[bSlotId].bSlotIdInOtherLocation]
            .ItemObject.ubNumberOfObjects +=
            ArmsDealerOfferArea[bSlotId].ItemObject.ubNumberOfObjects;
      }

      // Clear the flag that hatches out the item
      gpTempDealersInventory[ArmsDealerOfferArea[bSlotId].bSlotIdInOtherLocation].uiFlags &=
...
void SetSkiFaceRegionHelpText(INVENTORY_IN_SLOT *pInv, MOUSE_REGION *pRegion, UINT8 ubScreenArea) {
  ...
  if ((pInv != NULL) && (pInv->ubIdOfMercWhoOwnsTheItem != NO_PROFILE)) {
    ...
    // add who owns it
    swprintf(zHelpText, L"%s%s %s", gMercProfiles[pInv->ubIdOfMercWhoOwnsTheItem].zNickname,
             pMessageStrings[MSG_DASH_S], zTempText);
  } else {
    zHelpText[0] = '\0';
  }
...
INT8 AddItemToPlayersOfferArea(UINT8 ubProfileID, INVENTORY_IN_SLOT *pInvSlot,
  ...
  for (bCnt = 0; bCnt < SKI_NUM_TRADING_INV_SLOTS; bCnt++) {
    ...
    if (PlayersOfferArea[bCnt].fActive == FALSE) {
      ...
      SetSkiRegionHelpText(&PlayersOfferArea[bCnt], &gPlayersOfferSlotsMouseRegions[bCnt],
                           PLAYERS_OFFER_AREA);
      SetSkiFaceRegionHelpText(&PlayersOfferArea[bCnt],
                               &gPlayersOfferSlotsSmallFaceMouseRegions[bCnt], PLAYERS_OFFER_AREA);
...
BOOLEAN RemoveItemFromPlayersOfferArea(INT8 bSlot) {
  ...
  if (PlayersOfferArea[bSlot].bSlotIdInOtherLocation == -1) {
    if (!SKITryToReturnInvToOwnerOrCurrentMerc(&(PlayersOfferArea[bSlot]))) {
      // failed to add item, inventory probably filled up or item is unowned and current merc
      // ineligible
      return (FALSE);
    }
  }
...
void DisplayPlayersOfferArea() {
  ...
  for (sCnt = 0; sCnt < SKI_NUM_TRADING_INV_SLOTS; sCnt++) {
    if (PlayersOfferArea[sCnt].fActive) {
      ...
      if (Item[PlayersOfferArea[sCnt].sItemIndex].usItemClass == IC_MONEY) {
        ...
        if (PlayersOfferArea[sCnt].bSlotIdInOtherLocation != -1 &&
            PlayersOfferArea[sCnt].ubIdOfMercWhoOwnsTheItem != NO_PROFILE) {
          INT16 sSoldierID;

          sSoldierID = GetSoldierIDFromMercID(PlayersOfferArea[sCnt].ubIdOfMercWhoOwnsTheItem);
          Assert(sSoldierID != -1);

          PlayersOfferArea[sCnt].ItemObject.uiMoneyAmount =
              Menptr[sSoldierID].inv[PlayersOfferArea[sCnt].bSlotIdInOtherLocation].uiMoneyAmount;
      ...
      //			fDisplayHatchOnItem = ( PlayersOfferArea[ sCnt ].uiFlags & (
      // ARMS_INV_PLAYERS_ITEM_HAS_BEEN_EVALUATED | ARMS_INV_JUST_PURCHASED ) ) == 0;

      // Display the inventory slot
      DisplayInvSlot((UINT8)sCnt, PlayersOfferArea[sCnt].sItemIndex, usPosX, usPosY,
                     &PlayersOfferArea[sCnt].ItemObject, fDisplayHatchOnItem, PLAYERS_OFFER_AREA);
  ...
  if (gubSkiDirtyLevel == SKI_DIRTY_LEVEL2) {
    ...
    InsertCommasForDollarFigure(zTemp);
    InsertDollarSignInToString(zTemp);
    DrawTextToScreen(zTemp, SKI_PLAYERS_TOTAL_VALUE_X, (UINT16)(SKI_PLAYERS_TOTAL_VALUE_Y + 5),
...
UINT32 CalculateTotalArmsDealerCost() {
  ...
  for (uiCnt = 0; uiCnt < SKI_NUM_TRADING_INV_SLOTS; uiCnt++) {
    if (ArmsDealerOfferArea[uiCnt].fActive) {
      // if the dealer repairs
      if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS)
        uiTotal += CalculateObjectItemRepairCost(gbSelectedArmsDealerID,
                                                 &(ArmsDealerOfferArea[uiCnt].ItemObject));
      else
        uiTotal +=
            CalcShopKeeperItemPrice(DEALER_SELLING, FALSE, ArmsDealerOfferArea[uiCnt].sItemIndex,
                                    ArmsDealerInfo[gbSelectedArmsDealerID].dSellModifier,
                                    &ArmsDealerOfferArea[uiCnt].ItemObject);
    }
  }
...
UINT32 CalculateTotalPlayersValue() {
  ...
  for (ubCnt = 0; ubCnt < SKI_NUM_TRADING_INV_SLOTS; ubCnt++) {
    if ((PlayersOfferArea[ubCnt].fActive) &&
        (PlayersOfferArea[ubCnt].uiFlags & ARMS_INV_PLAYERS_ITEM_HAS_VALUE)) {
...
void PerformTransaction(UINT32 uiMoneyFromPlayersAccount) {
  ...
  } else  // not a bribe
  {
    ...
    if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer != ARMS_DEALER_REPAIRS) {
      ...
      // always reserve an empty slot for change.  We can't tell for sure whether player is getting
      // change because that could depend on how many items he's trying to buy won't fit - he could
      // only be getting change because of that!
    ...
    if (uiArmsDealersItemsCost > uiPlayersTotalMoneyValue) {
      ...
      if (uiArmsDealersItemsCost > uiPlayersTotalMoneyValue + LaptopSaveInfo.iCurrentBalance) {
        ...
        SpecialCharacterDialogueEvent(DIALOGUE_SPECIAL_EVENT_SKIP_A_FRAME, 0, 0, 0,
                                      giShopKeeperFaceIndex, DIALOGUE_SHOPKEEPER_UI);
        ...
        if (uiPlayersTotalMoneyValue)
          SpecialCharacterDialogueEvent(DIALOGUE_SPECIAL_EVENT_SHOPKEEPER, 1,
                                        (uiArmsDealersItemsCost - uiPlayersTotalMoneyValue), 0,
                                        giShopKeeperFaceIndex, DIALOGUE_SHOPKEEPER_UI);
        else
          SpecialCharacterDialogueEvent(DIALOGUE_SPECIAL_EVENT_SHOPKEEPER, 2,
                                        (uiArmsDealersItemsCost - uiPlayersTotalMoneyValue), 0,
                                        giShopKeeperFaceIndex, DIALOGUE_SHOPKEEPER_UI);
      }

      SpecialCharacterDialogueEvent(DIALOGUE_SPECIAL_EVENT_SHOPKEEPER, 7, 0, 0,
                                    giShopKeeperFaceIndex, DIALOGUE_SHOPKEEPER_UI);

      gfResetShopKeepIdleQuote = TRUE;

      // disable further calls to PerformTransaction() until callback calls us again to free this
      // flag
    ...
    if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
      ...
      if ((uiMoneyFromPlayersAccount + uiMoneyInPlayersOfferArea) >= uiArmsDealersItemsCost) {
        // Move all the players evaluated items to the dealer ( For a repairman, this means just:
    ...
    {
      // DON'T include uiMoneyFromPlayersAccount in change given to player.  The only way this can
      // happen is if he agrees to subtract from his balance, but there isn't enough room.  In that
      // case, the cost of items not transfered is used to reduce his balance deduction instead (not
      ...
      if (ArmsDealerInfo[gbSelectedArmsDealerID].uiFlags & ARMS_DEALER_GIVES_CHANGE) {
        if (iChangeToGiveToPlayer > 0) {
          ...
          /*
                                                  //Remove the change the arms dealer is returning
             to the player if( ( gArmsDealerStatus[ gbSelectedArmsDealerID ].uiArmsDealersCash -
             uiChangeToGiveToPlayer ) >= 0 ) gArmsDealerStatus[ gbSelectedArmsDealerID
             ].uiArmsDealersCash -= uiChangeToGiveToPlayer; else gArmsDealerStatus[
      ...
      if (gbSelectedArmsDealerID == ARMS_DEALER_TONY) gfHavePurchasedItemsFromTony = TRUE;

      // if the arms dealer is Howard( 125), set fact 222
      if (gbSelectedArmsDealerID == ARMS_DEALER_HOWARD) SetFactTrue(222);
    }
  ...
  for (uiCnt = 0; uiCnt < SKI_NUM_TRADING_INV_SLOTS; uiCnt++) {
    ...
    if (ArmsDealerOfferArea[uiCnt].fActive) {
      ...
      if (bSlotID != -1) {
        ...
        if ((gbSelectedArmsDealerID == ARMS_DEALER_FRANZ) &&
            (ArmsDealerOfferArea[uiCnt].sItemIndex == VIDEO_CAMERA)) {
          // set a special flag
      ...
      RemoveItemFromDealersInventory(&ArmsDealerOfferArea[uiCnt],
                                     (UINT8)ArmsDealerOfferArea[uiCnt].bSlotIdInOtherLocation);

      // Remove the items from the Shopkeepers Offer area
      if (!RemoveItemFromArmsDealerOfferArea(
              (UINT8)uiCnt, FALSE))  // ArmsDealerOfferArea[uiCnt].bSlotIdInOtherLocation
        Assert(0);

      if (ArmsDealerOfferArea[uiCnt].fActive) Assert(0);
    }
  }
...
BOOLEAN RemoveItemFromDealersInventory(INVENTORY_IN_SLOT *pInvSlot, UINT8 ubSlot) {
  ...
  sInvSlot = ubSlot;
  //	sInvSlot = ( gSelectArmsDealerInfo.ubCurrentPage - 1 ) * SKI_NUM_ARMS_DEALERS_INV_SLOTS +
  // ubSlot;
      ...
      if (PlayersOfferArea[uiCnt].uiFlags & ARMS_INV_PLAYERS_ITEM_HAS_VALUE) {
        ...
        if (Item[PlayersOfferArea[uiCnt].sItemIndex].usItemClass == IC_MONEY) {
          ...
          gArmsDealerStatus[gbSelectedArmsDealerID].uiArmsDealersCash +=
              PlayersOfferArea[uiCnt].ItemObject.uiMoneyAmount;
        } else {
          ...
          if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer != ARMS_DEALER_BUYS_ONLY) {
            ...
            AddObjectToArmsDealerInventory(gbSelectedArmsDealerID,
                                           &(PlayersOfferArea[uiCnt].ItemObject));
          }
        }
  ...
  */
  ...
  switch (ubSource) {
    ...
    case ARMS_DEALER_OFFER_AREA:
      ...
      // Restrict the cursor to the arms dealer offer area, players offer area and the players
      // inventory
    ...
    case PLAYERS_OFFER_AREA:
      ...
      if (PlayersOfferArea[bSlotNum].ItemObject.ubNumberOfObjects > MAX_OBJECTS_PER_SLOT) {
        ...
        // (sure it kind of sucks, but it's a lot easier than handling overloaded cursor objects in
        // Interface Items!
    ...
    case PLAYERS_INVENTORY:
      ...
      // First try to evaluate the item immediately like any other left-click on a merc's inventory
      // slot would be.  Only if that doesn't work (because there isn't enough room in the player's
      // offer area), the item is picked up into the cursor, and may then get placed into the
      // player's offer area directly, but it will NOT get evaluated that way, and so has no
      // possibility of entering the dealer's inventory (where complex items aren't permitted).
      ...
      } else  // not supposed to offer it, or dealer has no more room
      {
        ...
        // We have to do this before memsetting gMoveingItem, 'cause during swaps, gpItemPointer ==
        // &gMoveingItem.ItemObject!
        CopyObj(gpItemPointer, &TempObject);

        // ARM: The memset was previously commented out, in order to preserve the owning merc's inv
        // slot # during a swap of
        // items in an inventory slot.  However, that leads to other bugs: if you picked the thing
        // you're swapping in from a restricted inv slot (headgear, vest, etc.), the item swapped
        // out will end up belonging to an illegal slot, and return there with a right click on it
        // in the player's offer area.  So now ALL items picked up here are unowned.
        memset(&gMoveingItem, 0, sizeof(INVENTORY_IN_SLOT));
        ...
        // By necessity, these items don't belong to a slot (so you can't return them via a right
        // click), because it would be too much work to handle attachments, members of a stack, or
        // even items swapped out of slots.
...
void SetSkiCursor(UINT16 usCursor) {
  ...
  if (usCursor == EXTERN_CURSOR) {
    ...
    if (gInvDesc.uiFlags & MSYS_REGION_EXISTS) MSYS_ChangeRegionCursor(&gInvDesc, usCursor);
  ...
  switch (ubWhere) {
    ...
    case ARMS_DEALER_OFFER_AREA:
      ...
      if (ArmsDealerOfferArea[ubSpotLocation].fActive == FALSE) {
        ...
        SetSkiRegionHelpText(&ArmsDealerOfferArea[ubSpotLocation],
                             &gDealersOfferSlotsMouseRegions[ubSpotLocation],
                             ARMS_DEALER_OFFER_AREA);
        SetSkiFaceRegionHelpText(&ArmsDealerOfferArea[ubSpotLocation],
                                 &gDealersOfferSlotsSmallFaceMouseRegions[ubSpotLocation],
                                 ARMS_DEALER_OFFER_AREA);
    ...
    case PLAYERS_OFFER_AREA:
      ...
      if (PlayersOfferArea[ubSpotLocation].fActive == FALSE) {
        ...
        if (Item[PlayersOfferArea[ubSpotLocation].sItemIndex].usItemClass == IC_MONEY) {
          ...
          PlayersOfferArea[ubSpotLocation].uiItemPrice =
              PlayersOfferArea[ubSpotLocation].ItemObject.uiMoneyAmount;
        }

        SetSkiRegionHelpText(&PlayersOfferArea[ubSpotLocation],
                             &gPlayersOfferSlotsMouseRegions[ubSpotLocation], PLAYERS_OFFER_AREA);
        SetSkiFaceRegionHelpText(&PlayersOfferArea[ubSpotLocation],
                                 &gPlayersOfferSlotsSmallFaceMouseRegions[ubSpotLocation],
                                 PLAYERS_OFFER_AREA);
...
void HandleShopKeeperDialog(UINT8 ubInit) {
  ...
  if (ubInit == 1) {
    // special: if it's Arnie, and we have stuff in for repairs, but it's not fixed yet, use a
    // different opening quote!
    ...
    if (gfStartWithRepairsDelayedQuote) {
      ...
      // treat this as having done business - the player WAS here for a good reason, and it's the
      // dealers fault...
      gfDoneBusinessThisSession = TRUE;
    } else {
  ...
  // if the player has requested to leave
    ...
    if (!gFacesData[giShopKeeperFaceIndex].fTalking) {
      SpecialCharacterDialogueEvent(DIALOGUE_SPECIAL_EVENT_SHOPKEEPER, 5, 0, 0,
                                    giShopKeeperFaceIndex, DIALOGUE_SHOPKEEPER_UI);
    }
    ...
    if ((uiCurTime - uiLastTime) > (guiRandomQuoteDelayTime + Random(guiRandomQuoteDelayTime))) {
      ...
      if (Chance(35)) {
        ...
        {
          // if there is any items in the player's area with value to this dealer other than just
          // money
...
BOOLEAN StartShopKeeperTalking(UINT16 usQuoteNum) {
  ...
  SpecialCharacterDialogueEvent(DIALOGUE_SPECIAL_EVENT_SHOPKEEPER, 3, usQuoteNum, 0,
                                giShopKeeperFaceIndex, DIALOGUE_SHOPKEEPER_UI);

  // post quote dialogue
  fSuccess = CharacterDialogue(ArmsDealerInfo[gbSelectedArmsDealerID].ubShopKeeperID, usQuoteNum,
                               giShopKeeperFaceIndex, DIALOGUE_SHOPKEEPER_UI, FALSE, FALSE);

  // post event to mark shopkeeper dialogue as ended
  SpecialCharacterDialogueEvent(DIALOGUE_SPECIAL_EVENT_SHOPKEEPER, 4, usQuoteNum, 0,
                                giShopKeeperFaceIndex, DIALOGUE_SHOPKEEPER_UI);
...
BOOLEAN IsGunOrAmmoOfSameTypeSelected(OBJECTTYPE *pItemObject) {
  ...
  if (gpHighLightedItemObject == NULL) return (FALSE);
  ...
  if (Item[gpHighLightedItemObject->usItem].usItemClass == IC_AMMO) {
    ...
    if (Item[pItemObject->usItem].usItemClass == IC_GUN) {
      // of the same caliber
      if (Weapon[pItemObject->usItem].ubCalibre ==
          Magazine[Item[gpHighLightedItemObject->usItem].ubClassIndex].ubCalibre) {
        return (TRUE);
      }
    }
  }
  ...
  // else if the item is a gun
    ...
    if (Item[pItemObject->usItem].usItemClass == IC_AMMO) {
      // of the same caliber
      if (Weapon[gpHighLightedItemObject->usItem].ubCalibre ==
          Magazine[Item[pItemObject->usItem].ubClassIndex].ubCalibre) {
        return (TRUE);
      }
    }
  }
  ...
  if (Item[pItemObject->usItem].fFlags & ITEM_ATTACHMENT) {
    if (ValidAttachment(pItemObject->usItem, gpHighLightedItemObject->usItem)) return (TRUE);
  } else {
    if (ValidAttachment(gpHighLightedItemObject->usItem, pItemObject->usItem)) return (TRUE);
  }
...
void CreateSkiAtmButtons() {
  ...
  for (ubCnt = SKI_ATM_1; ubCnt <= SKI_ATM_9; ubCnt++) {
    guiSKI_AtmButton[ubCnt] = CreateIconAndTextButton(
        guiSKI_AtmNumButtonImage, SkiAtmText[ubCnt], SKI_ATM_BUTTON_FONT, SKI_ATM_BUTTON_COLOR,
        NO_SHADOW, SKI_ATM_BUTTON_COLOR, NO_SHADOW, TEXT_CJUSTIFIED, usPosX, usPosY, BUTTON_TOGGLE,
        MSYS_PRIORITY_HIGH + 2, DEFAULT_MOVE_CALLBACK, BtnSKI_AtmButtonCallback);
  ...
  usPosX = SKI_ATM_BUTTON_X;
  guiSKI_AtmButton[SKI_ATM_0] = CreateIconAndTextButton(
      guiSKI_AtmOkButtonImage, SkiAtmText[SKI_ATM_0], SKI_ATM_BUTTON_FONT, SKI_ATM_BUTTON_COLOR,
      NO_SHADOW, SKI_ATM_BUTTON_COLOR, NO_SHADOW, TEXT_CJUSTIFIED, usPosX, usPosY, BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH + 2, DEFAULT_MOVE_CALLBACK, BtnSKI_AtmButtonCallback);
  ...
  guiSKI_AtmButton[SKI_ATM_OK] = CreateIconAndTextButton(
      guiSKI_AtmOkButtonImage, SkiAtmText[SKI_ATM_OK], SKI_ATM_BUTTON_FONT, SKI_ATM_BUTTON_COLOR,
      NO_SHADOW, SKI_ATM_BUTTON_COLOR, NO_SHADOW, TEXT_CJUSTIFIED, usPosX, usPosY, BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH + 2, DEFAULT_MOVE_CALLBACK, BtnSKI_AtmButtonCallback);
  ...
  for (ubCnt = SKI_ATM_TAKE; ubCnt <= SKI_ATM_CLEAR; ubCnt++) {
    guiSKI_AtmButton[ubCnt] = CreateIconAndTextButton(
        guiSKI_AtmSideMenuButtonImage, SkiAtmText[ubCnt], SKI_ATM_BUTTON_FONT, SKI_ATM_BUTTON_COLOR,
        NO_SHADOW, SKI_ATM_BUTTON_COLOR, NO_SHADOW, TEXT_CJUSTIFIED, usPosX, usPosY, BUTTON_TOGGLE,
        MSYS_PRIORITY_HIGH + 2, DEFAULT_MOVE_CALLBACK, BtnSKI_AtmButtonCallback);
...
void AddNumberToSkiAtm(UINT8 ubNumber) {
  ...
  if (wcslen(gzSkiAtmTransferString) >= 9) return;

  // if its the first number being added and the number is a zero
  if (gzSkiAtmTransferString[0] == L'\0' && ubNumber == SKI_ATM_0) return;
...
void DisplaySkiAtmTransferString() {
  ...
  DrawTextToScreen(zSkiAtmTransferString, SKI_TRANSFER_STRING_X, SKI_TRANSFER_STRING_Y,
                   SKI_TRANSFER_STRING_WIDTH, SKI_ATM_BUTTON_FONT, FONT_MCOLOR_WHITE,
  ...
  DrawTextToScreen(zSkiAtmTransferString, SKI_TRANSFER_STRING_X, SKI_MERCS_MONEY_Y,
                   SKI_TRANSFER_STRING_WIDTH, SKI_ATM_BUTTON_FONT, FONT_MCOLOR_WHITE,
                   FONT_MCOLOR_BLACK, FALSE, RIGHT_JUSTIFIED);
}
...
void EnableDisableSkiAtmButtons() {
  ...
  switch (gubCurrentSkiAtmMode) {
    case SKI_ATM_DISABLED_MODE:
      ...
      for (ubCnt = SKI_ATM_0; ubCnt <= SKI_ATM_9; ubCnt++) DisableButton(guiSKI_AtmButton[ubCnt]);
    ...
    case SKI_ATM_TAKE_MODE:
      ...
      for (ubCnt = SKI_ATM_0; ubCnt <= SKI_ATM_9; ubCnt++) EnableButton(guiSKI_AtmButton[ubCnt]);
...
void HandleCurrentModeText(UINT8 ubMode) {
    ...
    if (ubLastMode != SKI_ATM_ERR_TAKE_MODE && ubLastMode != SKI_ATM_ERR_GIVE_MODE) {
      ...
      if ((uiCurTime - uiLastTime) > DELAY_PER_MODE_CHANGE_IN_ATM) {
        ...
        if (ubLastMode == ubMode)
          if (ubMode == ubDisplayMode) ubMode = SKI_ATM_DISPLAY_PLAYERS_BALANCE;
  ...
  DisplayWrappedString(SKI_MODE_TEXT_X, SKI_MODE_TEXT_Y, SKI_MODE_TEXT_WIDTH, 2,
                       SKI_ATM_BUTTON_FONT, FONT_MCOLOR_WHITE, zTemp, FONT_MCOLOR_BLACK, FALSE,
                       CENTER_JUSTIFIED);

  // invalidate the atm panel area
  InvalidateRegion(SKI_ATM_PANEL_X, SKI_ATM_PANEL_Y, SKI_ATM_PANEL_X + SKI_ATM_PANEL_WIDTH,
                   SKI_ATM_PANEL_Y + SKI_ATM_PANEL_HEIGHT);
}
...
void EnableDisableEvaluateAndTransactionButtons() {
  ...
  for (ubCnt = 0; ubCnt < SKI_NUM_TRADING_INV_SLOTS; ubCnt++) {
    ...
    if (PlayersOfferArea[ubCnt].fActive) {
      ...
      if (PlayersOfferArea[ubCnt].uiFlags & ARMS_INV_PLAYERS_ITEM_HAS_VALUE) {
        ...
        if (Item[PlayersOfferArea[ubCnt].sItemIndex].usItemClass != IC_MONEY) fItemEvaluated = TRUE;
  ...
  */
  ...
  if (fItemEvaluated) {
    ...
    /*
                    //if the only item in the players offer area is money, and there is nothing in
       the dealers offer area if( IsMoneyTheOnlyItemInThePlayersOfferArea( ) &&
       uiArmsDealerTotalCost == 0 ) DisableButton( guiSKI_TransactionButton );
    */
  ...
  /* Allow transaction attempt when dealer can't afford to buy that much - he'll reject it with a
     special quote! else if( ( gArmsDealerStatus[ gbSelectedArmsDealerID ].uiArmsDealersCash +
     uiArmsDealerTotalCost ) < ( uiPlayersOfferAreaTotalCost - uiPlayersOfferAreaTotalMoney ) )
          {
                  DisableButton( guiSKI_TransactionButton );
          }
  */
...
BOOLEAN IsMoneyTheOnlyItemInThePlayersOfferArea() {
  ...
  if (fFoundMoney) Assert(CalculateHowMuchMoneyIsInPlayersOfferArea() > 0);
...
void MovePlayersItemsToBeRepairedToArmsDealersInventory() {
  ...
  for (uiCnt = 0; uiCnt < SKI_NUM_TRADING_INV_SLOTS; uiCnt++) {
    ...
    if (ArmsDealerOfferArea[uiCnt].fActive) {
      ...
      //				d) If a gun, stripped of any non-ammo-class GunAmmoItems,
      // and bullets

      // add it to the arms dealer's inventory
      GiveObjectToArmsDealerForRepair(gbSelectedArmsDealerID,
                                      &(ArmsDealerOfferArea[uiCnt].ItemObject),
                                      ArmsDealerOfferArea[uiCnt].ubIdOfMercWhoOwnsTheItem);
...
void EvaluateItemAddedToPlayersOfferArea(INT8 bSlotID, BOOLEAN fFirstOne) {
  ...
  if (Item[PlayersOfferArea[bSlotID].sItemIndex].usItemClass == IC_MONEY) return;
  ...
  if (WillShopKeeperRejectObjectsFromPlayer(gbSelectedArmsDealerID, bSlotID) == FALSE) {
    ...
    if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
      ...
      ubNumberOfItemsAddedToRepairDuringThisEvaluation =
          PlayersOfferArea[bSlotID].ItemObject.ubNumberOfObjects;

      // if there is already enough items in for repairs, complain about it and DON'T accept the
      // item for repairs
      ...
      // if the item is damaged, or is a rocket rifle (which always "need repairing" even at 100%,
      // to reset imprinting)
      if ((PlayersOfferArea[bSlotID].ItemObject.bStatus[0] < 100) || fRocketRifleWasEvaluated) {
        INT8 bSlotAddedTo;

        // Move the item to the Dealer's Offer Area
        bSlotAddedTo = AddItemToArmsDealerOfferArea(
            &PlayersOfferArea[bSlotID], PlayersOfferArea[bSlotID].bSlotIdInOtherLocation);
        ...
        if (bSlotAddedTo != -1) {
          ...
          /*	ARM: Leave it there, until transaction occurs it should be recallable
                                                  //Remove the item from the owner merc's inventory
                                                  IfMercOwnedRemoveItemFromMercInv(
             &(ArmsDealerOfferArea[ bSlotAddedTo ]) );
          */
          ...
          if (Item[ArmsDealerOfferArea[bSlotAddedTo].sItemIndex].usItemClass != IC_AMMO) {
            if (ArmsDealerOfferArea[bSlotAddedTo].ItemObject.bStatus[0] <
                REALLY_BADLY_DAMAGED_THRESHOLD) {
...
#ifdef JA2BETAVERSION
          ScreenMsg(FONT_MCOLOR_WHITE, MSG_BETAVERSION,
                    L"Failed to add repair item to ArmsDealerOfferArea.  AM-0");
#endif
          return;
        }
      } else {
  ...
  // when evaluating complex items that get split into multiple subobjects, dealer will talk only
  // about the first one! don't bother with any of this if shopkeeper can't talk right now
    ...
    switch (uiEvalResult) {
      case EVAL_RESULT_DONT_HANDLE:
        if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_SELLS_ONLY) {
          ...
          // while the text of 18 seems wrong for Sam & Howard if offered something they should
          // consider valuable.
      ...
      case EVAL_RESULT_NON_REPAIRABLE:
        // use quote 4 for this situation...  It's a bit weird but better than "that looks fine /
        // ain't broke"
        sQuoteNum = SK_QUOTES_NOT_INTERESTED_IN_THIS_ITEM;
        break;
      ...
      default:
#ifdef JA2BETAVERSION
        ScreenMsg(FONT_MCOLOR_WHITE, MSG_BETAVERSION, L"Invalid evaluation result of %d.  AM-0",
                  uiEvalResult);
#endif
        break;
    }
...
BOOLEAN DoSkiMessageBox(UINT8 ubStyle, STR16 zString, UINT32 uiExitScreen, UINT8 ubFlags,
  ...
  giSKIMessageBox = DoMessageBox(ubStyle, zString, uiExitScreen,
...
BOOLEAN WillShopKeeperRejectObjectsFromPlayer(INT8 bDealerId, INT8 bSlotId) {
  ...
  if (Item[PlayersOfferArea[bSlotId].sItemIndex].usItemClass == IC_MONEY) {
    fRejected = FALSE;
  } else if (CanDealerTransactItem(gbSelectedArmsDealerID, PlayersOfferArea[bSlotId].sItemIndex,
                                   TRUE)) {
    fRejected = FALSE;
  }
...
void InitShopKeeperItemDescBox(OBJECTTYPE *pObject, UINT8 ubPocket, UINT8 ubFromLocation) {
  ...
  switch (ubFromLocation) {
    case ARMS_DEALER_INVENTORY: {
      ...
      // if the box will appear over the mercs face, move the box over so it doesn't obstruct the
      // face
...
BOOLEAN OfferObjectToDealer(OBJECTTYPE *pComplexObject, UINT8 ubOwnerProfileId, INT8 bOwnerSlotId) {
  ...
  } else  // dealing with a repairman
  {
    ...
    // determine how many subobjects of each category this complex object will have to be broken up
    // into
    CountSubObjectsInObject(pComplexObject, &ubTotalSubObjects, &ubRepairableSubObjects,
                            &ubNonRepairableSubObjects);

    // in the simplest situation, the # subobjects of each type gives us how many slots of each type
    // are needed
    ubDealerOfferAreaSlotsNeeded = ubRepairableSubObjects;
    ubPlayerOfferAreaSlotsNeeded = ubNonRepairableSubObjects;

    // consider that if dealer is at or will reach his max # of items repaired limit, not everything
    // repairable will move
    ...
    // if he can't repair everything repairable that we're about to submit, the space we'll need
    // changes
    ...
    if (ubDealerOfferAreaSlotsNeeded > 0) {
      // we need at least one EXTRA empty slot in player's area to pass repairable objects through
    ...
    /*
                    This code is commented out because a repair dealer will never be allowed to
       repair over more then SKI_MAX_AMOUNT_OF_ITEMS_DEALER_CAN_REPAIR_AT_A_TIME ( 4 ) items at a
       time, therefore, cant fill up the dealer offer area

                    // check if we have room for all of each type
                    if ( ( ubDealerOfferAreaSlotsNeeded +
       CountNumberOfItemsInTheArmsDealersOfferArea() ) > SKI_NUM_TRADING_INV_SLOTS )
                    {
                            // tell player there's not enough room in the dealer's offer area
                            DoSkiMessageBox( MSG_BOX_BASIC_STYLE, L"There is not enough room in the
    ...
    */
    ...
    for (ubSubObject = 0; ubSubObject < MAX_SUBOBJECTS_PER_OBJECT; ubSubObject++) {
      ...
      if (gSubObject[ubSubObject].usItem != NONE) {
        // if it's the main item itself (always in the very first subobject), and it has no other
        // subobjects
        ...
        {
          // store it with a valid owner, but an invalid slot, so it still shows who owns it, but
          // can't return to its slot
  ...
  // ARM: This comment isn't true unless parent/child support is added.  Right now repairmen don't
  // do this!
  // NOTE that, either way, if owned, the item remains in the merc's inventory (hatched out) until
...
void SplitComplexObjectIntoSubObjects(OBJECTTYPE *pComplexObject) {
  ...
  if (pComplexObject->ubNumberOfObjects == 1) {
    ...
    if (Item[pComplexObject->usItem].usItemClass == IC_GUN) {
      ...
      /* gunAmmoStatus is currently not being used that way, it's strictly used as a
         jammed/unjammed, and so should never be 0
    ...
    /* ARM: Can now repair with removeable attachments still attached...
                    ...
                    for( ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++ )
                    {
                            if ( pComplexObject->usAttachItem[ ubCnt ] != NONE )
                            {
                                    ...
                                    if (! (Item[ pComplexObject->usAttachItem[ ubCnt ] ].fFlags &
       ITEM_INSEPARABLE ) )
                                    {
    ...
    */
    ...
    /* ARM: Can now repair with removeable attachments still attached...
       ...
       ITEM_INSEPARABLE ) )
                                    {
                                            CreateItem( pComplexObject->usAttachItem[ ubCnt ],
       pComplexObject->bAttachStatus[ ubCnt ], pNextObj );
...
BOOLEAN ShopkeeperAutoPlaceObject(SOLDIERCLASS *pSoldier, OBJECTTYPE *pObject, BOOLEAN fNewItem) {
  ...
  while (ubObjectsLeftToPlace > 0) {
    // figure out how many to place during this loop iteration.  Can't do more than
    // MAX_OBJECTS_PER_SLOT at a time
...
// The Shopkeeper interface *MUST* use this intermediary function instead of calling AddItemToPool()
// directly! This is because the OBJECTTYPEs used within Shopkeeper may contain an illegal
// ubNumberOfObjects
void ShopkeeperAddItemToPool(INT16 sGridNo, OBJECTTYPE *pObject, INT8 bVisible, UINT8 ubLevel,
                             UINT16 usFlags, INT8 bRenderZHeightAboveLevel) {
  OBJECTTYPE CopyOfObject;
...
BOOLEAN SKITryToReturnInvToOwnerOrCurrentMerc(INVENTORY_IN_SLOT *pInv) {
  ...
  if (pInv->ubIdOfMercWhoOwnsTheItem != NO_PROFILE) {
    ...
    // Try to find a place to put in its owner's inventory (regardless of which merc is currently
    // displayed!)
...
BOOLEAN SKITryToAddInvToMercsInventory(INVENTORY_IN_SLOT *pInv, SOLDIERCLASS *pSoldier) {
  ...
  if (Item[pInv->sItemIndex].usItemClass == IC_MONEY) {
    ...
    if (bMoneyInvPos != -1) {
      // try to add to it.  If successful, it will delete the object...  It returns TRUE even if not
      // all would fit!
...
BOOLEAN CanMercInteractWithSelectedShopkeeper(SOLDIERCLASS *pSoldier) {
  ...
  if (pSoldier->bActive && pSoldier->bInSector && IsMercOnCurrentSquad(pSoldier) &&
    ...
    if (SoldierTo3DLocationLineOfSightTest(pSoldier, sDestGridNo, bDestLevel, 3,
      ...
      // and is close enough to talk to the shopkeeper (use this define INSTEAD of
      // PASSING_ITEM_DISTANCE_OKLIFE!)
...
void ReturnItemToPlayerSomehow(INVENTORY_IN_SLOT *pInvSlot, SOLDIERCLASS *pDropSoldier) {
  ...
  if (pInvSlot->bSlotIdInOtherLocation == -1) {
    ...
    if (!SKITryToReturnInvToOwnerOrCurrentMerc(pInvSlot)) {
      // failed to add item, inventory probably filled up or item is unowned and current merc
      // ineligible. drop it at the specified guy's feet instead
      ShopkeeperAddItemToPool(pDropSoldier->sGridNo, &pInvSlot->ItemObject, VISIBLE,
                              pDropSoldier->bLevel, 0, 0);
    }
  }
}
...
void GivePlayerSomeChange(UINT32 uiAmount) {
  ...
  if ((gbSelectedArmsDealerID == ARMS_DEALER_JAKE) &&
    ...
    // HACK HACK HACK: We forgot to write/record quote 27 for Jake, so he ALWAYS must have enough
    // money!
    gArmsDealerStatus[gbSelectedArmsDealerID].uiArmsDealersCash +=
...
void DealerGetsBribed(UINT8 ubProfileId, UINT32 uiMoneyAmount) {
  ...
  switch (ubProfileId) {
    case FRANK:
      fBribable = TRUE;
      uiMinBribe = 10;  // This should match the handling of a bribe submitted outside the SKI,
                        // using the GIVE system
...
void HandlePossibleRepairDelays() {
  ...
  if ((gbSelectedArmsDealerID == ARMS_DEALER_FREDO) ||
      (gbSelectedArmsDealerID == ARMS_DEALER_PERKO)) {
    ...
    if (!gArmsDealerStatus[gbSelectedArmsDealerID].fRepairDelayBeenUsed) {
      ...
      if ((GetWorldTotalMin() - gArmsDealerStatus[gbSelectedArmsDealerID].uiTimePlayerLastInSKI) >=
        ...
        if (RepairmanFixingAnyItemsThatShouldBeDoneNow(
                &uiHoursSinceAnyItemsShouldHaveBeenRepaired) &&
            (uiHoursSinceAnyItemsShouldHaveBeenRepaired < REPAIR_DELAY_IN_HOURS)) {
          // then there's a fair chance he'll be delayed.  Use pre-chance to hopefully preserve
          // across reloads
          if (PreChance(50)) {
        ...
        if (pSpecialItem->fActive) {
          ...
          if (pSpecialItem->Info.bItemCondition < 0) {
            ...
            if (pSpecialItem->uiRepairDoneTime <= GetWorldTotalMin()) {
              ...
              // we need to determine how long it's been since the item that's been repaired for the
              // longest time was done
...
void SelectArmsDealersDropItemToGroundRegionCallBack(MOUSE_REGION *pRegion, INT32 iReason) {
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
    ...
    if (gMoveingItem.sItemIndex != 0) {
      ...
      ShopkeeperAddItemToPool(pDropSoldier->sGridNo, &gMoveingItem.ItemObject, VISIBLE,
                              pDropSoldier->bLevel, 0, 0);
...
void DisplayTheSkiDropItemToGroundString() {
  ...
  usHeight =
      DisplayWrappedString(SKI_DROP_ITEM_TO_GROUND_START_X, SKI_DROP_ITEM_TO_GROUND_TEXT_START_Y,
                           (UINT16)(640 - SKI_DROP_ITEM_TO_GROUND_START_X), 2, SKI_LABEL_FONT,
  ...
  DisplayWrappedString(SKI_DROP_ITEM_TO_GROUND_START_X,
                       (UINT16)(SKI_DROP_ITEM_TO_GROUND_TEXT_START_Y - usHeight),
                       (UINT16)(640 - SKI_DROP_ITEM_TO_GROUND_START_X), 2, SKI_LABEL_FONT,
...
UINT32 EvaluateInvSlot(INVENTORY_IN_SLOT *pInvSlot) {
  ...
  if (uiBuyingPrice > 0) {
    ...
    // he normally buys this, but it's in such bad shape that he won't, it's worthless (different
    // quote)
