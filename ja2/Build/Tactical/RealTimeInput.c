                      ...
                      if (pSoldier->bDoBurst) {
                        ...
                        *puiNewEvent = A_CHANGE_TO_CONFIM_ACTION;
                        ...
                        if (UIMouseOnValidAttackLocation(pSoldier)) {
                          // OK< going into confirm will call a function that will automatically
                          // move us to shoot in most vases ( grenades need a confirm mode
                          // regardless )
                          *puiNewEvent = A_CHANGE_TO_CONFIM_ACTION;
            ...
            if (fValidDoubleClickPossible) {
              if (gpItemPointer == NULL) {
                ...
                if (gusSelectedSoldier != NO_SOLDIER) {
                  ...
                  // ANIM_STATIONARY ) if ( MercPtrs[ gusSelectedSoldier ]->usAnimState == WALKING )
                  {
                      ...
                      if (gfUIWaitingForUserSpeechAdvance && fCanCheckForSpeechAdvance) {
                        ...
                        switch (gCurrentUIMode) {
                          ...
                          case CONFIRM_ACTION_MODE:
                            ...
                            //	if ( gusSelectedSoldier != NO_SOLDIER )
                            //	{
                            //		if ( !( gAnimControl[ MercPtrs[ gusSelectedSoldier
                            //]->usAnimState
                                      ...
                                      if (!_KeyDown(ALT)) {
                                        ...
                                        *puiNewEvent = I_SELECT_MERC;
                                        ...
                                        if (pSoldier->uiStatusFlags & SOLDIER_MULTI_SELECTED) {
                                          ...
                                          if (gusSelectedSoldier != NOBODY) {
                                            MercPtrs[gusSelectedSoldier]->uiStatusFlags |=
                                  ...
                                  if (GetSoldier(&pSoldier, gusSelectedSoldier)) {
                                    ...
                                    if (FindSoldierFromMouse(&usSoldierIndex, &uiMercFlags) &&
                                      ...
                                      *puiNewEvent = I_SELECT_MERC;
                                      ...
                                      // if ( FindBestPath( pSoldier, usMapPos, pSoldier->bLevel,
                                      // pSoldier->usUIMovementMode, NO_COPYROUTE, 0 ) == 0 )
                                      if (gsCurrentActionPoints == 0 && !gfUIAllMoveOn &&
                                        ...
                                        if (gusSelectedSoldier != NOBODY) {
                                          if ((fResult = UIOKMoveDestination(
                                                   MercPtrs[gusSelectedSoldier], usMapPos)) == 1) {
                                            if (gfUIAllMoveOn) {
            ...
            if (gfUIFullTargetFound) {
              if (IsValidTalkableNPC((UINT8)gusUIFullTargetID, FALSE, FALSE, FALSE) &&
                ...
                *puiNewEvent = T_CHANGE_TO_TALKING;
                ...
                if ((guiUIFullTargetFlags & OWNED_MERC) && (guiUIFullTargetFlags & VISIBLE_MERC) &&
                    !(guiUIFullTargetFlags & DEAD_MERC)) {
                  // uiMoveTargetSoldierId = gusUIFullTargetID;
                  //*puiNewEvent = A_ON_TERRAIN;
                  // return;
