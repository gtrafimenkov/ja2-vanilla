...
void StartInterrupt(void) {
  ...
  for (cnt = gubOutOfTurnPersons; cnt > 0; cnt--) {
    DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
             String("STARTINT:  Q position %d: %d", cnt, gubOutOfTurnOrder[cnt]));
  }

  // DebugMsg( TOPIC_JA2, DBG_LEVEL_3, String("INTERRUPT: %d is now on top of the interrupt queue",
  // ubFirstInterrupter ) );
  ...
  if (pSoldier->bTeam == OUR_TEAM) {
    ...
    while (1) {
      ...
      DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
               String("INTERRUPT: popping %d off of the interrupt queue", ubInterrupter));
    ...
    DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
             String("INTERRUPT: starting interrupt for %d", ubFirstInterrupter));
...
void EndInterrupt(BOOLEAN fMarkInterruptOccurred) {
  ...
  for (cnt = gubOutOfTurnPersons; cnt > 0; cnt--) {
    DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
             String("ENDINT:  Q position %d: %d", cnt, gubOutOfTurnOrder[cnt]));
  }
  ...
  if (!EveryoneInInterruptListOnSameTeam()) {
    ...
    DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
             String("INTERRUPT: interrupt over, %d's team regains control", ubInterruptedSoldier));
...
BOOLEAN StandardInterruptConditionsMet(SOLDIERTYPE *pSoldier, UINT8 ubOpponentID,
...
#ifdef BETAVERSION
      NumMessage(
          "StandardInterruptConditions: ERROR - ubOpponentID is NOBODY, don't know who threw rock, "
          "guynum = ",
          pSoldier->guynum);
#endif
  ...
  if (ubOpponentID < NOBODY) {
    ...
    if ((ubOpponentID != gusSelectedSoldier) && (pSoldier->bSide !=
    Menptr[gusSelectedSoldier].bSide))
    {
            return(FALSE);
    }
    */
  ...
  if (pSoldier->bPassedLastInterrupt) {
    ...
    fprintf(NetDebugFile,
            "\tStandardInterruptConditionsMet: FAILING because PassedLastInterrupt %d(%s)\n",
...
INT8 CalcInterruptDuelPts(SOLDIERTYPE *pSoldier, UINT8 ubOpponentID, BOOLEAN fUseWatchSpots) {
  ...
  if (fUseWatchSpots) {
    ...
    bPoints += GetWatchedLocPoints(pSoldier->ubID, MercPtrs[ubOpponentID]->sGridNo,
  ...
  if ((gTacticalStatus.uiFlags & INCOMBAT) && (pSoldier->bTeam != gTacticalStatus.ubCurrentTeam)) {
    ...
    // unfortunately we can't use opplist here to record whether or not we saw this guy before,
    // because at this point the opplist has been updated to seen.  But we can use gbSeenOpponents
    // ...
...
#ifdef BETAVERSION
    NumMessage("CalcInterruptDuelPts: ERROR - Invalid bInterruptDuelPts calculated for soldier ",
               pSoldier->guynum);
#endif
...
void AddToIntList(UINT8 ubID, BOOLEAN fGainControl, BOOLEAN fCommunicate) {
  ...
  DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
           String("INTERRUPT: adding ID %d who %s", ubID,
    ...
    for (ubTeam = 0; ubTeam < MAXTEAMS; ubTeam++) {
      if (gTacticalStatus.Team[ubTeam].bTeamActive &&
        ...
        for (ubOpp = gTacticalStatus.Team[ubTeam].bFirstID;
          ...
          if (pOpponent->bActive && pOpponent->bInSector && (pOpponent->bLife >= OKLIFE) &&
...
#ifdef DEBUG_INTERRUPTS
                DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                         String("Resetting int pts for %d - NOISE BEYOND SIGHT DISTANCE!?",
                                pOpponent->ubID));
#endif
                continue;
              }
...
#ifdef DEBUG_INTERRUPTS
              DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                       String("Resetting int pts for %d - DOESN'T SEE ON SIGHT INTERRUPT!?",
                              pOpponent->ubID));
#endif
...
// either way, clear out both sides' bInterruptDuelPts field to prepare next one
    ...
    if (ubIntCnt) {
      ...
      if ((gTacticalStatus.ubCurrentTeam != pSoldier->bTeam) &&
        ...
        for (ubLoop = gTacticalStatus.Team[gTacticalStatus.ubCurrentTeam].bFirstID;
             ubLoop <= gTacticalStatus.Team[gTacticalStatus.ubCurrentTeam].bLastID; ubLoop++) {
