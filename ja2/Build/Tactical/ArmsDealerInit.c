...
// Once allocated, the special item slots remain allocated for the duration of the game, or until
// the dealer dies. This is a little bit wasteful, but saves an awful lot of hassles, and avoid
...
ARMS_DEALER_INFO ArmsDealerInfo[NUM_ARMS_DEALERS] = {
    ...
    // Speed		Cost
    /* Arnie Brunzwell*/
    {0.1f, 0.8f, ARNIE, ARMS_DEALER_REPAIRS, 1500,
     ARMS_DEALER_HAS_NO_INVENTORY | ARMS_DEALER_GIVES_CHANGE},
    /* Fredo */
    {0.6f, 0.6f, FREDO, ARMS_DEALER_REPAIRS, 1000,
     ARMS_DEALER_HAS_NO_INVENTORY | ARMS_DEALER_GIVES_CHANGE},
    /* Perko */
    {1.0f, 0.4f, PERKO, ARMS_DEALER_REPAIRS, 1000,
     ARMS_DEALER_HAS_NO_INVENTORY | ARMS_DEALER_GIVES_CHANGE},
...
void AddSpecialItemToArmsDealerInventoryAtElement(UINT8 ubArmsDealer, UINT16 usItemIndex,
                                                  UINT8 ubElement,
                                                  SPECIAL_ITEM_INFO *pSpclItemInfo);

void RemoveRandomItemFromArmsDealerInventory(UINT8 ubArmsDealer, UINT16 usItemIndex,
                                             UINT8 ubHowMany);
...
BOOLEAN DoesItemAppearInDealerInventoryList(UINT8 ubArmsDealer, UINT16 usItemIndex,
...
// INT16 GetSpecialItemFromArmsDealerInventory( UINT8 ubArmsDealer, UINT16 usItemIndex,
// SPECIAL_ITEM_INFO *pSpclItemInfo );
...
void InitializeOneArmsDealer(UINT8 ubArmsDealer) {
  ...
  for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
    ...
    if (CanDealerTransactItem(ubArmsDealer, usItemIndex, FALSE)) {
      // Setup an initial amount for the items (treat items as new, how many are used isn't known
      // yet)
  ...
  for (ubArmsDealer = 0; ubArmsDealer < NUM_ARMS_DEALERS; ubArmsDealer++) {
    ...
    for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
      ...
      if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubElementsAlloced > 0) {
        ...
        if (!AllocMemsetSpecialItemArray(
    ...
    for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
      ...
      if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems > 0) {
        ...
        if (usItemIndex == JAR_ELIXIR) {
          // only allow selling of standard # of items so those converted from blood given by player
          // will be available
        ...
        // next, try to sell all the used ones, gotta do these one at a time so we can remove them
        // by element
        for (ubElement = 0;
      ...
      if (CanDealerTransactItem(ubArmsDealer, usItemIndex, FALSE)) {
        ...
        if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubQtyOnOrder == 0) {
          ...
          if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems <=
            ...
            // determine if the item can be restocked (assume new, use items aren't checked for
            // until the stuff arrives)
            if (ItemTransactionOccurs(ubArmsDealer, usItemIndex, DEALER_BUYING, FALSE)) {
              // figure out how many items to reorder (items are reordered an entire batch at a
              // time)
              ...
              if (!fPrevElig) {
                ...
                // This is just a way to reward the player right away for making progress without
                // the reordering lag...
                ArmsDealerGetsFreshStock(ubArmsDealer, usItemIndex, ubNumItems);
              } else {
                if ((ubArmsDealer == ARMS_DEALER_TONY) || (ubArmsDealer == ARMS_DEALER_DEVIN)) {
                  // the stuff Tony and Devin sell is imported, so it takes longer to arrive (for
                  // game balance)
              ...
              if( gArmsDealersInventory[ ubArmsDealer ][ usItemIndex ].ubTotalItems )
              {
                      ...
                      if( uiDealerItemType & GetArmsDealerItemTypeFromItemNumber( usItemIndex ) )
                      {
                              ...
                              if( Random( 100 ) < 30 )
                              {
                                      ...
                                      if( uiDealerItemType == ARMS_DEALER_AMMO )
                                      {
                                              // remove all of them, since each ammo item counts as
      only one "item" here

                                              // create item info describing a perfect item
                                              SetSpecialItemInfoToDefaults( &SpclItemInfo );
                                              // ammo will always be only condition 100, there's
      never any in special slots RemoveItemFromArmsDealerInventory( ubArmsDealer, usItemIndex,
      &SpclItemInfo, gArmsDealersInventory[ ubArmsDealer ][ usItemIndex ].ubTotalItems );
                                      }
                                      else
                                      {
                                              // pick 1 random one, don't care about its condition
                                              RemoveRandomItemFromArmsDealerInventory( ubArmsDealer,
      usItemIndex, 1 );
                                      }
...
void GuaranteeAtLeastOneItemOfType(UINT8 ubArmsDealer, UINT32 uiDealerItemType) {
  ...
  // if there aren't any such items, the following loop would never finish, so quit before trying
  // it!
...
void GuaranteeAtLeastXItemsOfIndex(UINT8 ubArmsDealer, UINT16 usItemIndex, UINT8 ubHowMany) {
  ...
  // if he can stock it (it appears in his inventory list)
  // RESTRICTION REMOVED: Jake must be able to guarantee GAS even though it's not in his list, it's
...
UINT32 GetArmsDealerItemTypeFromItemNumber(UINT16 usItem) {
  switch (Item[usItem].usItemClass) {
    ...
    default:
      AssertMsg(
          FALSE,
          String("GetArmsDealerItemTypeFromItemNumber(), invalid class %d for item %d.  DF 0.",
...
BOOLEAN IsMercADealer(UINT8 ubMercID) {
  ...
  if (ubMercID == GABBY) return (FALSE);
#endif
...
SPECIAL_ITEM_INFO *pSpclItemInfo )
{
        ...
        for( ubElement = 0; ubElement < gArmsDealersInventory[ ubArmsDealer ][ usItemIndex
].ubElementsAlloced; ubElement++ )
        {
                // if this is the one we're looking for
                if( memcmp( &(gArmsDealersInventory[ ubArmsDealer ][ usItemIndex ].SpecialItem[
ubElement ].Info), pSpclItemInfo, sizeof( SPECIAL_ITEM_INFO ) ) == 0 )
                {
                        return( ubElement );
                }
        }
    ...
    case ARMS_DEALER_BUYS_SELLS:
      switch (ubArmsDealer) {
        ...
        case ARMS_DEALER_FRANZ:
          if (fPurchaseFromPlayer) {
            // these guys will buy nearly anything from the player, regardless of what they carry
            // for sale!
...
BOOLEAN CanDealerRepairItem(UINT8 ubArmsDealer, UINT16 usItemIndex) {
  ...
  switch (ubArmsDealer) {
    ...
    default:
      AssertMsg(
          FALSE,
          String("CanDealerRepairItem(), Arms Dealer %d is not a recognized repairman!.  AM 1.",
                 ubArmsDealer));
  }
...
BOOLEAN AllocMemsetSpecialItemArray(DEALER_ITEM_HEADER *pDealerItem, UINT8 ubElementsNeeded) {
  ...
  pDealerItem->SpecialItem =
      (DEALER_SPECIAL_ITEM *)MemAlloc(sizeof(DEALER_SPECIAL_ITEM) * ubElementsNeeded);
...
BOOLEAN ResizeSpecialItemArray(DEALER_ITEM_HEADER *pDealerItem, UINT8 ubElementsNeeded) {
  ...
  pDealerItem->SpecialItem = (DEALER_SPECIAL_ITEM *)MemRealloc(
      pDealerItem->SpecialItem, sizeof(DEALER_SPECIAL_ITEM) * ubElementsNeeded);
  if (pDealerItem->SpecialItem == NULL) {
    Assert(0);
    return (FALSE);
  }
...
UINT32 CountTotalItemsInArmsDealersInventory( UINT8 ubArmsDealer )
{
        ...
        for( usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++ )
        {
                // This counts each pack of ammo or stacked item as one.  See the "distinct" version
of this for an alternate version uiNumOfItems += gArmsDealersInventory[ ubArmsDealer ][ usItemIndex
].ubTotalItems;
        }
    ...
    if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems > 0) {
      ...
      if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubPerfectItems > 0) {
        ...
        if (DealerItemIsSafeToStack(usItemIndex)) {
          // regardless of how many there are, they count as 1 *distinct* item!  They will all be
          // together in one box...
          uiNumOfItems++;
        } else {
      ...
      // each *active* special item counts as one additional distinct item (each one occupied a
      // separate shopkeeper box!) NOTE: This is including items being repaired!!!
...
UINT8 CountActiveSpecialItemsInArmsDealersInventory(UINT8 ubArmsDealer, UINT16 usItemIndex) {
  ...
  // next, try to sell all the used ones, gotta do these one at a time so we can remove them by
  // element
  for (ubElement = 0;
       ubElement < gArmsDealersInventory[ubArmsDealer][usItemIndex].ubElementsAlloced;
       ubElement++) {
...
void AddObjectToArmsDealerInventory(UINT8 ubArmsDealer, OBJECTTYPE *pObject) {
  ...
  switch (Item[pObject->usItem].usItemClass) {
    case IC_GUN:
      ...
      // if the gun was jammed, this will forget about the jam (i.e. dealer immediately unjams
      // anything he buys)
    ...
    default:
      // add each object seperately (multiple objects may have vastly different statuses, keep any
      // imprintID)
      for (ubCnt = 0; ubCnt < pObject->ubNumberOfObjects; ubCnt++) {
...
void AddAmmoToArmsDealerInventory(UINT8 ubArmsDealer, UINT16 usItemIndex, UINT8 ubShotsLeft) {
  ...
  if (ubShotsLeft > 0) {
    ...
    *pubStrayAmmo += ubShotsLeft;
    ...
    // I know, I know, this is getting pretty anal...  But what the hell, it was easy enough to do.
    // ARM.
  }
}
...
void AddItemToArmsDealerInventory(UINT8 ubArmsDealer, UINT16 usItemIndex,
  ...
  if (IsItemInfoSpecial(pSpclItemInfo)) {
    // Anything that's used/damaged or imprinted is store as a special item in the SpecialItem
    // array, exactly one item per element.  We (re)allocate memory dynamically as necessary to hold
    ...
    do {
      ...
      if (!fFoundOne) {
        ...
        if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubElementsAlloced == 0) {
          ...
          fSuccess = AllocMemsetSpecialItemArray(&gArmsDealersInventory[ubArmsDealer][usItemIndex],
                                                 ubElementsToAdd);
        } else {
          // we have some allocated, but they're all full and we need more.  MemRealloc existing
          // amount + # addition elements
        ...
        // now add the special item at the first of the newly added elements (still stored in
        // ubElement!)
        AddSpecialItemToArmsDealerInventoryAtElement(ubArmsDealer, usItemIndex, ubElement,
                                                     pSpclItemInfo);
      }

      // store the # of the element it was placed in globally so anyone who needs that can grab it
      // there
...
void AddSpecialItemToArmsDealerInventoryAtElement(UINT8 ubArmsDealer, UINT16 usItemIndex,
  ...
  memcpy(&(gArmsDealersInventory[ubArmsDealer][usItemIndex].SpecialItem[ubElement].Info),
         pSpclItemInfo, sizeof(SPECIAL_ITEM_INFO));
...
void RemoveRandomItemFromArmsDealerInventory(UINT8 ubArmsDealer, UINT16 usItemIndex,
  ...
  // not permitted for repair dealers - would take extra code to subtract items under repair from
  // ubTotalItems!!!
  Assert(!DoesDealerDoRepairs(ubArmsDealer));
  ...
  while (ubHowMany > 0) {
    ...
    if (ubWhichOne < gArmsDealersInventory[ubArmsDealer][usItemIndex].ubPerfectItems) {
      ...
      // Yikes!  Gotta look through the special items.  We already know it's not any of the perfect
      // ones, subtract those
      ...
      for (ubElement = 0;
        ...
        // usItemIndex
        //].SpecialItem[ ubElement ].Info.bItemCondition > 0 ) )
        {
...
void RemoveSpecialItemFromArmsDealerInventoryAtElement(UINT8 ubArmsDealer, UINT16 usItemIndex,
                                                       UINT8 ubElement) {
...
BOOLEAN AddDeadArmsDealerItemsToWorld(UINT8 ubMercID) {
  ...
  if (pSoldier == NULL) {
    // This should never happen, a dealer getting knocked off without the sector being loaded,
    // should it? If it's possible, we should modify code below to dump his belongings into the
    // sector without using pSoldier->sGridNo
    Assert(0);
    return (FALSE);
  }

  // loop through all the items in the dealer's inventory, and drop them all where the dealer was
  // set up.
  ...
  for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
    ...
    if (gArmsDealersInventory[bArmsDealer][usItemIndex].ubTotalItems > 0) {
      ...
      if (gArmsDealersInventory[bArmsDealer][usItemIndex].ubPerfectItems > 0) {
        ...
        // drop stackable items like ammo in stacks of whatever will fit into a large pocket instead
        // of one at a time
...
void MakeObjectOutOfDealerItems(UINT16 usItemIndex, SPECIAL_ITEM_INFO *pSpclItemInfo,
  ...
  if (Item[pObject->usItem].usItemClass == IC_GUN) {
    ...
    // is so that we don't have to keep track of #bullets in a gun throughout dealer inventory.
    // Without this, players could "reload" guns they don't have ammo for by selling them to Tony &
    // buying them right back fully loaded!  One could repeat this ad nauseum (empty the gun between
    // visits) as a (really expensive) way to get unlimited special ammo like rockets.
...
void GiveObjectToArmsDealerForRepair(UINT8 ubArmsDealer, OBJECTTYPE *pObject,
  ...
  */
  ...
  // if the gun was jammed, this will forget about the jam (i.e. dealer immediately unjams anything
...
UINT32 CalculateSimpleItemRepairTime(UINT8 ubArmsDealer, UINT16 usItemIndex, INT8 bItemCondition) {
  ...
  // first calc what he'll charge - that takes care of item condition, repair ease, and his repair
  // cost "markup"
  ...
  // For a repairman, his BUY modifier controls his REPAIR SPEED (1.0 means minutes to repair =
  ...
  // charge double) (Mind you, current he's the ONLY one who CAN repair Electronics at all!  Oh
  // well.)
...
UINT32 CalculateSpecialItemRepairCost(UINT8 ubArmsDealer, UINT16 usItemIndex,
  ...
  uiRepairCost =
      CalculateSimpleItemRepairCost(ubArmsDealer, usItemIndex, pSpclItemInfo->bItemCondition);
  ...
  for (ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++) {
    if (pSpclItemInfo->usAttachment[ubCnt] != NONE) {
      ...
      if ((pSpclItemInfo->bAttachmentStatus[ubCnt] < 100) &&
          CanDealerRepairItem(ubArmsDealer, pSpclItemInfo->usAttachment[ubCnt])) {
        uiRepairCost +=
            CalculateSimpleItemRepairCost(ubArmsDealer, pSpclItemInfo->usAttachment[ubCnt],
                                          pSpclItemInfo->bAttachmentStatus[ubCnt]);
      }
    }
  }
...
BOOLEAN DoesItemAppearInDealerInventoryList(UINT8 ubArmsDealer, UINT16 usItemIndex,
  ...
  while (pDealerInv[usCnt].sItemIndex != LAST_DEALER_ITEM) {
    ...
    if (pDealerInv[usCnt].sItemIndex == usItemIndex) {
      // if optimal quantity listed is 0, it means dealer won't sell it himself, but will buy it
      // from the player!
...
UINT16 CalcValueOfItemToDealer(UINT8 ubArmsDealer, UINT16 usItemIndex, BOOLEAN fDealerSelling) {
  ...
  // Micky & Gabby specialize in creature parts & such, the others don't buy these at all
  // (exception: jars)
  if ((usItemIndex != JAR) &&
...
BOOLEAN DealerItemIsSafeToStack(UINT16 usItemIndex) {
  // basically any item type with nothing unique about it besides its status can be stacked in
  // dealer's inventory boxes... NOTE: This test is only applied to items already KNOWN to be
  // perfect - special items are obviously not-stackable
...
UINT32 CalculateMinutesClosedBetween(UINT8 ubArmsDealer, UINT32 uiStartTime, UINT32 uiEndTime) {
  ...
  // NOTE: this assumes stored are only closed overnight, so all we have to do is compare the day
  // portion
  ...
  // treat end time of midnight as 24:00 hours to prevent indefinite recursion and make formulas
  // work
  if (uiEndTime == 0) {
