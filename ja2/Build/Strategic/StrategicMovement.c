...
void SetLocationOfAllPlayerSoldiersInGroup(GROUP *pGroup, INT16 sSectorX, INT16 sSectorZ,
                                           INT8 bSectorZ);
...
BOOLEAN AddWaypointToPGroup(GROUP *pGroup, UINT8 ubSectorX,
                            UINT8 ubSectorY)  // Same, but overloaded
{
  WAYPOINT *pWay;
...
void HandleImportantPBIQuote(SOLDIERCLASS *pSoldier, GROUP *pInitiatingBattleGroup) {
  ...
  if (pSoldier->fMercAsleep) {
    ...
    TacticalCharacterDialogueWithSpecialEvent(pSoldier, QUOTE_ENEMY_PRESENCE,
                                              ...
                                              (UINT32)pInitiatingBattleGroup, 0);
    TacticalCharacterDialogueWithSpecialEvent(pSoldier, QUOTE_ENEMY_PRESENCE,
...
void TriggerPrebattleInterface(UINT8 ubResult) {
  ...
  SpecialCharacterDialogueEvent(DIALOGUE_SPECIAL_EVENT_TRIGGERPREBATTLEINTERFACE,
                                (UINT32)gpInitPrebattleGroup, 0, 0, 0, 0);
...
void GroupArrivedAtSector(UINT8 ubGroupID, BOOLEAN fCheckForBattle, BOOLEAN fNeverLeft) {
  ...
  if (pGroup->fPlayer) {
    if (pGroup->ubSectorZ == 0) {
      SectorInfo[SECTOR(pGroup->ubSectorX, pGroup->ubSectorY)].bLastKnownEnemies =
...
BOOLEAN PlayerGroupInMotion(GROUP *pGroup) { return (pGroup->fBetweenSectors); }
    ...
    if (curr->fPlayer == TRUE) {
      ...
      if (!fHelicopterGroup || !fShowAircraftFlag) {
        ...
        if ((gfDisplayPotentialRetreatPaths == TRUE) && (sBattleSector == sSource) ||
          ...
          if ((sBattleSector == sSource) && (SECTOR(curr->ubSectorX, curr->ubSectorY) == sSource) &&
              (SECTOR(curr->ubPrevX, curr->ubPrevY) == sDest)) {
            fMayRetreatFromBattle = TRUE;
          }

          if ((sBattleSector == sDest) && (SECTOR(curr->ubSectorX, curr->ubSectorY) == sDest) &&
