...
void CalculateMedicalDepositRefund(SOLDIERCLASS *pSoldier);
void NotifyPlayerOfMercDepartureAndPromptEquipmentPlacement(SOLDIERCLASS *pSoldier,
                                                            BOOLEAN fAddRehireButton);
void MercDepartEquipmentBoxCallBack(UINT8 bExitValue);
BOOLEAN HandleFiredDeadMerc(SOLDIERCLASS *pSoldier);
void HandleExtendMercsContract(SOLDIERCLASS *pSoldier);
void HandleSoldierLeavingWithLowMorale(SOLDIERCLASS *pSoldier);
void HandleSoldierLeavingForAnotherContract(SOLDIERCLASS *pSoldier);
// BOOLEAN SoldierWantsToDelayRenewalOfContract( SOLDIERCLASS *pSoldier );
void HandleNotifyPlayerCantAffordInsurance(void);
void HandleNotifyPlayerCanAffordInsurance(SOLDIERCLASS *pSoldier, UINT8 ubLength, INT32 iCost);
void ExtendMercInsuranceContractCallBack(UINT8 bExitValue);
void HandleUniqueEventWhenPlayerLeavesTeam(SOLDIERCLASS *pSoldier);

UINT32 uiContractTimeMode = 0;

SOLDIERCLASS *pLeaveSoldier = NULL;
...
SOLDIERCLASS *pContractReHireSoldier = NULL;

UINT8 gubContractLength = 0;  // used when extending a mercs insurance contract
SOLDIERCLASS *gpInsuranceSoldier = NULL;
...
BOOLEAN SaveContractRenewalDataToSaveGameFile(HWFILE hFile) {
  ...
  MemFileWrite(hFile, ContractRenewalList, sizeof(ContractRenewalList), &uiNumBytesWritten);
  if (uiNumBytesWritten != sizeof(ContractRenewalList)) {
    return (FALSE);
  }

  MemFileWrite(hFile, &ubNumContractRenewals, sizeof(ubNumContractRenewals), &uiNumBytesWritten);
...
BOOLEAN WillMercRenew(SOLDIERCLASS *pSoldier, BOOLEAN fSayQuote) {
  ...
  if (fSayQuote) {
    if (fUnhappy) {
      if (fBuddyAround) {
        if (GetMercPrecedentQuoteBitStatus(pSoldier->ubProfile,
                                           GetQuoteBitNumberFromQuoteID((UINT32)(usBuddyQuote))) ==
            TRUE) {
          fSayPrecedent = TRUE;
        } else {
          SetMercPrecedentQuoteBitStatus(pSoldier->ubProfile,
                                         GetQuoteBitNumberFromQuoteID((UINT32)(usBuddyQuote)));
        }
      } else {
        if (GetMercPrecedentQuoteBitStatus(pSoldier->ubProfile,
...
void HandleSoldierLeavingForAnotherContract(SOLDIERCLASS *pSoldier) {
  if (pSoldier->fSignedAnotherContract) {
    ...
    gMercProfiles[pSoldier->ubProfile].uiDayBecomesAvailable +=
...
void CalculateMedicalDepositRefund(SOLDIERCLASS *pSoldier) {
  ...
  if (!gMercProfiles[pSoldier->ubProfile].bMedicalDeposit) return;
  ...
  if (pSoldier->bLife == pSoldier->bLifeMax) {
    ...
    AddEmailWithSpecialData(AIM_MEDICAL_DEPOSIT_REFUND, AIM_MEDICAL_DEPOSIT_REFUND_LENGTH, AIM_SITE,
                            GetWorldTotalMin(), pSoldier->usMedicalDeposit, pSoldier->ubProfile);
  }
  ...
  // else the player is injured, refund a partial amount
    ...
    AddEmailWithSpecialData(AIM_MEDICAL_DEPOSIT_PARTIAL_REFUND,
                            ...
                            iRefundAmount, pSoldier->ubProfile);
  }
}
...
void FindOutIfAnyMercAboutToLeaveIsGonnaRenew(void) {
  ...
  SOLDIERCLASS *pSoldier = NULL, *pSoldierWhoWillQuit = NULL;
